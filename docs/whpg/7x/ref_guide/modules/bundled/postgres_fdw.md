...
The `postgres_fdw` module is installed when you install WarehousePG. Before you can use the foreign-data wrapper, you must register the `postgres_fdw` extension in each database in which you want to use the foreign-data wrapper. Refer to [Installing Extensions](../../../install_guide/additional_modules/index.md) for more information.

<a id="using_mrs"></a>

## Using postgres_fdw to Access Multiple Remote PostgreSQL Servers

Before you [create a foreign table](../../sql_commands/CREATE_FOREIGN_TABLE.md) using the `postgres_fdw` foreign-data wrapper, you must configure a server with the [CREATE SERVER](../../sql_commands/CREATE_SERVER.md) command. You can use a `postgres_fdw` foreign table to access data that is distributed across multiple remote PostgreSQL servers when you set certain WarehousePG and `postgres_fdw`-specific options on the `CREATE SERVER` command:
...
For more information about using foreign tables in WarehousePG, see [Accessing External Data with Foreign Tables](../../../admin_guide/external/foreign/index.md).
