...
Use `pg_dump` if you are migrating your data to another database vendor's system, or to another WarehousePG cluster with a different segment configuration (for example, if the system you are migrating to has greater or fewer segment instances). To restore, you must use the corresponding [pg_restore](pg_restore.md) utility (if the dump file is in archive format), or you can use a client program such as [psql](psql.md) (if the dump file is in plain text format).
...
-   c \| custom — Output a custom archive suitable for input into [pg_restore](pg_restore.md). Together with the directory output format, this is the most flexible output format in that it allows manual selection and reordering of archived items during restore. This format is compressed by default.
...
-   t \| tar — Output a tar-format archive suitable for input into [pg_restore](pg_restore.md). The tar format is compatible with the directory format; extracting a tar-format archive produces a valid directory-format archive. However, the tar format does not support compression. Also, when using tar format the relative order of table data items cannot be changed during restore.
...
`pg_dump` will open \\&lt;njobs> + 1 connections to the database, so make sure your [max_connections](../../config_params/guc-list.md) setting is high enough to accommodate all connections.
...
[pg_dumpall](pg_dumpall.md), [pg_restore](pg_restore.md), [psql](psql.md)
