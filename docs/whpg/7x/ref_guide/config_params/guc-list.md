...
Selects the text search configuration that is used by those variants of the text search functions that do not have an explicit argument specifying the configuration. See [Using Full Text Search](../../admin_guide/query/textsearch/index.md) for further information. The built-in default is `pg_catalog.simple`, but `initdb` will initialize the configuration file with a setting that corresponds to the chosen `lc_ctype` locale, if a configuration matching that locale can be identified.
...
> **Note** Setting `default_transaction_deferrable` to `on` has no effect in WarehousePG. Only read-only, `SERIALIZABLE` transactions can be deferred. However, WarehousePG does not support the `SERIALIZABLE` transaction isolation level. See [SET TRANSACTION](../sql_commands/SET_TRANSACTION.md).
...
This parameter is ignored for a root partitioned table. When GPORCA is enabled and the root partition does not have statistics, GPORCA always uses the default value. You can use `ANALZYE ROOTPARTITION` to collect statistics on the root partition. See [ANALYZE](../sql_commands/ANALYZE.md).
...
For an example of setting `gp_interconnect_proxy_addresses`, see [Configuring Proxies for the WarehousePG Interconnect](../../admin_guide/managing/proxy-ic.md).
...
For information about the `RECURSIVE` keyword, see the [SELECT](../sql_commands/SELECT.md) command and [WITH Queries (Common Table Expressions)](../../admin_guide/query/CTE-query.md).
...
Identifies the resource management scheme currently enabled in the WarehousePG cluster. For information about WarehousePG resource management, see [Managing Resources](../../admin_guide/performance/wlmgmt/index.md).
...
Controls the time (in seconds) that WarehousePG waits before returning an error when WarehousePG is attempting to connect or write to a [gpfdist](../utility_guide/reference/gpfdist.md) server and `gpfdist` does not respond. The default value is 300 (5 minutes). A value of 0 deactivates the timeout.
...
Writes the generated LLVM IR out to the file system, inside [data_directory](../../install_guide/create_data_dirs.md). This is only useful for working on the internals of the JIT implementation. The default setting is `off`. Only superusers and users with the appropriate `SET` privilege can change this setting.
...
For information about the Postgres-based planner and GPORCA, see [SQL: Querying Data](../../admin_guide/query/index.md) in the *WarehousePG Administrator Guide*.
...
When the value of the server configuration parameter [optimizer](#optimizer) is `on` (the default), the value of this parameter should also be `on`. For information about collecting table statistics on partitioned tables, see [ANALYZE](../sql_commands/ANALYZE.md).
...
For information about GPORCA, see [About GPORCA](../../admin_guide/query/query-piv-optimizer/index.md) in the *WarehousePG Administrator Guide*.
...
For information about GPORCA, see the [About GPORCA](../../admin_guide/query/query-piv-optimizer/index.md) topic.
...
The allowed values are `auto` (the default), `force_custom_plan` and `force_generic_plan`. This setting is considered when a cached plan is to be run, not when it is prepared. For more information see [PREPARE](../sql_commands/PREPARE.md).
...
Ensures that all identifiers are quoted, even if they are not keywords, when the database generates SQL. See also the `--quote-all-identifiers` option of [pg_dump](../utility_guide/reference/pg_dump.md) and [pg_dumpall](../utility_guide/reference/pg_dumpall.md).
...
For more information about row-level security policies, see [CREATE POLICY](../sql_commands/CREATE_POLICY.md).
...
Enables timing of WAL I/O calls. This parameter is disabled by default, as it repeatedly queries the operating system for the current time, which may cause significant overhead on some platforms. The view [pg_stat_wal](../system_catalogs/catalog_ref-views.md#pg_stat_wal) displays WAL I/O timing information. Only superusers and users with the appropriate `SET` privilege can change this setting.
...
For information about the `gpfdists` protocol and how this setting affects the certificate files required on WarehousePG segments, see [gpfdists:// Protocol](../../admin_guide/external/external-tables/gpfdists-protocol.md). For information about running the `gpfdist` utility, see [gpfdist](../utility_guide/reference/gpfdist.md).
...
This parameter serves hot-standby dispatchers only. When set, dispatch resolves every content's address from this file instead of selecting a row from [gp_segment_configuration](../system_catalogs/system_catalogs_definitions/gp_segment_configuration.md), with fail-closed validation and a `dbid` handshake. On any other node, the component-table build refuses to start. Each row in the file must carry the node's own identity, the `gp_dbid` value from `internal.auto.conf`. A relative path is relative to the data directory.
