...
Additional restrictions apply when unique indexes are applied to partitioned tables; see [CREATE TABLE](CREATE_TABLE.md).
...
tablespace_name
	The tablespace in which to create the index. If not specified, [default_tablespace](../config_params/guc-list.md) is consulted, or [temp_tablespaces](../config_params/guc-list.md) for indexes on temporary tables.
...
The system regularly collects statistics on all of a table's columns. Newly-created non-expression indexes can immediately use these statistics to determine an index's usefulness. For new expression indexes, you must run [ANALYZE](ANALYZE.md) to generate statistics for these indexes.

For most index methods, the speed of creating an index is dependent on the setting of [maintenance_work_mem](../config_params/guc-list.md#maintenance_work_mem). Larger values will reduce the time needed for index creation, so long as you don't make it larger than the amount of memory really available, which would drive the machine into swapping.
...
Use [DROP INDEX](DROP_INDEX.md) to remove an index.
