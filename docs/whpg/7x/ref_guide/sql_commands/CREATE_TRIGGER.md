...
[SELECT](SELECT.md) does not modify any rows so you can not create `SELECT` triggers. Rules and views are more appropriate in such cases.
...
[CREATE FUNCTION](CREATE_FUNCTION.md), [ALTER TRIGGER](ALTER_TRIGGER.md), [DROP TRIGGER](DROP_TRIGGER.md), [CREATE RULE](CREATE_RULE.md)
