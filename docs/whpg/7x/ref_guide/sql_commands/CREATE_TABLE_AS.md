...
TEMPORARY \| TEMP
If specified, the new table is created as a temporary table. Refer to [CREATE TABLE](CREATE_TABLE.md) for details.

UNLOGGED
If specified, the table is created as an unlogged table. Refer to [CREATE TABLE](CREATE_TABLE.md) for details.
...
The `WITH` clause specifies optional storage parameters for the new table. Refer to the [Storage Parameters](CREATE_TABLE.md#storage_parameters) section on the `CREATE TABLE` reference page for more information.
...
DELETE ROWS — WarehousePG deletes all rows in the temporary table at the end of each transaction block. Essentially, an automatic [TRUNCATE](TRUNCATE.md) is done at each commit.

DROP — WarehousePG drops the temporary table at the end of the current transaction block.

TABLESPACE tablespace_name
The tablespace_name parameter is the name of the tablespace in which the new table is to be created. If not specified, the database's [default_tablespace](../config_params/guc-list.md#default_tablespace) is used, or [temp_tablespaces](../config_params/guc-list.md#temp_tablespaces) if the table is temporary.

AS query
A [SELECT](SELECT.md), [TABLE](SELECT.md#table-command), or [VALUES](VALUES.md) command, or an [EXECUTE](EXECUTE.md) command that runs a prepared `SELECT`, `TABLE`, or `VALUES` query.
...
-   WarehousePG handles temporary tables differently from the standard; see [CREATE TABLE](CREATE_TABLE.md) for details.
...
[CREATE EXTERNAL TABLE](CREATE_EXTERNAL_TABLE.md), [CREATE MATERIALIZED VIEW](CREATE_MATERIALIZED_VIEW.md), [CREATE TABLE](CREATE_TABLE.md), [EXECUTE](EXECUTE.md), [SELECT](SELECT.md), [SELECT INTO](SELECT_INTO.md), [VALUES](VALUES.md)
