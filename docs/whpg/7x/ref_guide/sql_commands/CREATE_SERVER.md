...
When using the dblink module (see [dblink](/extensions/bundled/dblink.md)), you can use the foreign server name as an argument of the `dblink_connect()` function to provide the connection parameters. You must have the `USAGE` privilege on the foreign server to use it in this manner.
...
[ALTER SERVER](ALTER_SERVER.md), [DROP SERVER](DROP_SERVER.md), [CREATE FOREIGN DATA WRAPPER](CREATE_FOREIGN_DATA_WRAPPER.md), [CREATE FOREIGN TABLE](CREATE_FOREIGN_TABLE.md), [CREATE USER MAPPING](CREATE_USER_MAPPING.md)
