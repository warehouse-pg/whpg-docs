...
`VACUUM ANALYZE` performs a `VACUUM` and then an `ANALYZE` for each selected table. This is a handy combination form for routine maintenance scripts. See [ANALYZE](ANALYZE.md) for more details about its processing.
...
Selects aggressive *freezing* of tuples. Specifying `FREEZE` is equivalent to performing `VACUUM` with the [vacuum_freeze_min_age](../config_params/guc-list.md#vacuum_freeze_min_age) and `vacuum_freeze_table_age` server configuration parameter set to zero. Aggressive freezing is always performed when the table is rewritten, so this option is redundant when `FULL` is specified.
...
`VACUUM` causes a substantial increase in I/O traffic, which might cause poor performance for other active sessions. Therefore, it is sometimes advisable to use the [cost-based vacuum delay](../config_params/guc_category-list.md#topic19).
...
For more information about concurrency control in WarehousePG, see [Routine System Maintenance Tasks](../../admin_guide/managing/maintain.md) in the *WarehousePG Administrator Guide*.
