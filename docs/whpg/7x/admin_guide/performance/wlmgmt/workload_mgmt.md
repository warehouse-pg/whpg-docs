...
-   If no `MEMORY_LIMIT` is set for a resource queue, the amount of memory allocated per query is the value of the [statement_mem](../../../ref_guide/config_params/guc-list.md#statement_mem) server configuration parameter. The maximum memory the resource queue can use is the product of `statement_mem` and `ACTIVE_STATEMENTS`.
...
See the [CREATE RESOURCE QUEUE](../../../ref_guide/sql_commands/CREATE_RESOURCE_QUEUE.md), [CREATE ROLE](../../../ref_guide/sql_commands/CREATE_ROLE.md), and [ALTER ROLE](../../../ref_guide/sql_commands/ALTER_ROLE.md) statements in the *WarehousePG Reference Guide* for help with command syntax and detailed reference information.
...
When you install WarehousePG, no resource management policy is enabled by default. To use resource queues, set the [gp_resource_manager](../../../ref_guide/config_params/guc-list.md#gp_resource_manager) server configuration parameter:
...
Before you create any resource groups, learn about the different resouce queue server configuration parameters and their usage. See [Server Configuration Parameters](../../../ref_guide/config_params/guc-list.md) for more information.
...
When you create a resource queue for a role, you provide a name, set an active query limit, and optionally a query priority for the resource queue. Use the [CREATE RESOURCE QUEUE](../../../ref_guide/sql_commands/CREATE_RESOURCE_QUEUE.md) command to create new resource queues.
...
The [gp_resqueue_status](../../../ref_guide/system_catalogs/catalog_ref-views.md#gp_resqueue_status) `gp_toolkit` view allows administrators to see status and activity for a resource queue. It shows how many queries are waiting to run and how many queries are currently active in the system from a particular resource queue. To see the resource queues created in the system, their limit attributes, and their current status:
...
To see the roles assigned to a resource queue, perform the following query of the `pg_roles` and The [gp_resqueue_status](../../../ref_guide/system_catalogs/catalog_ref-views.md#gp_resqueue_status) `gp_toolkit` system catalog tables:
...
The *gp_toolkit* administrative schema has a view called [gp_resq_priority_statement](../../../ref_guide/gp_toolkit.md#gp_resq_priority_statement), which lists all statements currently being run and provides the priority, session ID, and other information.
...
To obtain the session ID and statement count parameters required by this function, superusers can use the `gp_toolkit` administrative schema view [gp_resq_priority_statement](../../../ref_guide/gp_toolkit.md#gp_resq_priority_statement). From the view, use these values for the function parameters.
