...
The settings of the [verify_gpfdists_cert](../../../ref_guide/config_params/guc-list.md#verify_gpfdists_cert) server configuration parameter (default value `true`) and the [gpfdist](../../../ref_guide/utility_guide/reference/gpfdist.md) `--ssl_verify_peer <boolean>` option (default value `on`) control whether SSL certificate authentication is enabled when WarehousePG communicates with the `gpfdist` utility to either read data from or write data to an external data source. These settings also determine which of the following certificate files must reside in the `$PGDATA/gpfdists` directory on each WarehousePG segment:
