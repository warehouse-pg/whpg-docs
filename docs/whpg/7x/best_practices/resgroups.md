...
For conceptual information about configuring and using resource groups, see [Using Resource Groups](../admin_guide/performance/wlmgmt/workload_mgmt_resgroups.md).
...
-   **vm.overcommit_memory**
    ...
    This Linux kernel parameter, set in [`/etc/sysctl.conf`](../install_guide/config_os.md#topic3), identifies the method that the operating system uses to determine how much memory can be allocated to processes. `vm.overcommit_memory` must always be set to 2 for WarehousePG clusters.

-   **vm.overcommit_ratio**

    This Linux kernel parameter, set in [`/etc/sysctl.conf`](../install_guide/config_os.md#topic3), identifies the percentage of RAM that is used for application processes; the remainder is reserved for the operating system. Tune the setting as necessary. If your memory utilization is too low, increase the value; if your memory or swap usage is too high, decrease the setting.
...
-   Do not configure the operating system to use huge pages. See the [Recommended OS Parameters Settings](../install_guide/config_os.md#topic3) in the *WarehousePG Installation Guide*.
