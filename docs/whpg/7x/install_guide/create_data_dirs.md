...
1.  On the coordinator host, switch to the `gpadmin` user and source the `greenplum_path.sh` file.
    ...
    ::: info Note
    Logging in as `root` with `su` fails on hosts where root login is deactivated, such as Amazon EC2 instances. Use `sudo` for the `gpssh` commands on the remote hosts instead, so you can run them as `gpadmin`, provided `gpadmin` has `sudo` privilege as described in [Creating the WarehousePG Administrative User](config_os.md#topic23).
    :::
