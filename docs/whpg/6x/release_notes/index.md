...
| ------------------------------------- | --------------- |
| [6.27.6-WHPG](#warehousepg-6-27-6-whpg) | 25 September 2026 |
| [6.27.5-WHPG](#warehousepg-6-27-5-whpg) | 10 June 2026    |
...
-   Fixed a `pg_dump` privilege-escalation race by adding the [`restrict_nonsystem_relation_kind`](../ref_guide/config_params/guc-list.md#restrict_nonsystem_relation_kind) configuration parameter (`view`, `foreign-table`), which `pg_dump` sets for the duration of a dump to stop the backend from expanding non-system views or accessing non-system foreign tables, for [CVE-2024-7348](https://www.cve.org/CVERecord?id=CVE-2024-7348).
...
-   Fixed a one-byte overread in GB18030 multibyte character handling and replaced unbounded `pg_mblen()` call sites with length-checked variants across the server, for [CVE-2025-4207](https://www.cve.org/CVERecord?id=CVE-2025-4207) and [CVE-2026-2006](https://www.cve.org/CVERecord?id=CVE-2026-2006).
...
-   Fixed `pgcrypto` to fail cipher operations during PGP encryption instead of emitting effectively unencrypted data, and added the [`ignore-cipher-failure`](../security-guide/Encryption.md#recovering-data-encrypted-before-the-cve-2026-14663-fix) decryption option to read back data mis-encrypted by affected builds, for [CVE-2026-14663](https://www.cve.org/CVERecord?id=CVE-2026-14663).
...
-   Used overflow-safe allocation in `pltcl`, `plperl`, and `plpython`, and guarded fixed-size argument arrays in the parser, executor, fmgr, and these PL languages against extreme argument counts, for [CVE-2026-14677](https://www.cve.org/CVERecord?id=CVE-2026-14677) and [CVE-2026-14679](https://www.cve.org/CVERecord?id=CVE-2026-14679).
...
-   Rejected SQL-level calls to functions that take or return the `internal` type, including operator and cast syntax, and made aggregate combine functions return `NULL` honestly, for [CVE-2026-14680](https://www.cve.org/CVERecord?id=CVE-2026-14680).
