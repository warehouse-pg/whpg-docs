...
Refer to [Installing Extensions](../../../install_guide/additional_modules/index.md) for more information about installing and registering modules in WarehousePG.
...
To access a remote WarehousePG cluster, you must first create a foreign server object which specifies the host, port, and database connection details. You provide these connection parameters in the `OPTIONS` clause of the [CREATE SERVER](../../sql_commands/CREATE_SERVER.md) command.
...
After you identify which users you will allow to access the remote WarehousePG cluster, you must create one or more mappings between a local WarehousePG user and a user on the remote WarehousePG cluster. You create these mappings with the [CREATE USER MAPPING](../../sql_commands/CREATE_USER_MAPPING.md) command.
