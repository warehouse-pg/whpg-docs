...
If no time zone is specified in the input string, then it is presumed to be in the time zone indicated by the system's [`TIMEZONE` server configuration parameter](../../config_params/guc-list.md#timezone) and is converted to UTC using the offset for the time zone.
