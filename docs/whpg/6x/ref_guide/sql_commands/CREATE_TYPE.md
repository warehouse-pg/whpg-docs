...
It is possible to create an enumerated type with zero labels, but such a type cannot be used to hold values before at least one label is added using [ALTER TYPE](ALTER_TYPE.md).

### Range Types

The third form of `CREATE TYPE` creates a new range type, as described in [Range Types](../data_types/datatype-range.md).
...
-   category
    ...
    The category code (a single ASCII character) for this type. The default is '`U`', signifying a user-defined type. You can find the other standard category codes in [`pg_type` Category Codes](../system_catalogs/system_catalogs_definitions/pg_type.md#category-codes). You may also assign unused ASCII characters to custom categories that you create.
...
[ALTER TYPE](ALTER_TYPE.md), [CREATE DOMAIN](CREATE_DOMAIN.md), [CREATE FUNCTION](CREATE_FUNCTION.md), [DROP TYPE](DROP_TYPE.md)
