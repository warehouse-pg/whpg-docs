...
For writable S3 external tables, the `INSERT` operation uploads to one or more files in the configured S3 bucket, as described in [s3:// Protocol](../../admin_guide/external/external-tables/s3-protocol.md). Pressing `Ctrl-c` cancels the `INSERT` and stops uploading to S3.
...
[COPY](COPY.md), [SELECT](SELECT.md), [CREATE EXTERNAL TABLE](CREATE_EXTERNAL_TABLE.md), [s3:// Protocol](../../admin_guide/external/external-tables/s3-protocol.md)
