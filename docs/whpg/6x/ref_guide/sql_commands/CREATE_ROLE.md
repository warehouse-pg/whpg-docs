...
-   NOLOGIN
    ...
    If specified, `LOGIN` allows a role to log in to a database. A role having the `LOGIN` attribute can be thought of as a user. Roles with `NOLOGIN` are useful for managing database privileges, and can be thought of as groups. If not specified, `NOLOGIN` is the default, except when `CREATE ROLE` is invoked through its alternative spelling [CREATE USER](CREATE_USER.md).
...
The preferred way to add and remove role members (manage groups) is to use [GRANT](GRANT.md) and [REVOKE](REVOKE.md).
...
[SET ROLE](SET_ROLE.md), [ALTER ROLE](ALTER_ROLE.md), [DROP ROLE](DROP_ROLE.md), [GRANT](GRANT.md), [REVOKE](REVOKE.md), [CREATE RESOURCE QUEUE](CREATE_RESOURCE_QUEUE.md) [CREATE RESOURCE GROUP](CREATE_RESOURCE_GROUP.md)
