...
-   WITH ( storage_parameter \[= value] \[, ... ] )
    ...
    This clause specifies optional storage parameters for the materialized view. All parameters supported for `CREATE TABLE` are also supported for `CREATE MATERIALIZED VIEW` with the exception of OIDS. See [CREATE TABLE](CREATE_TABLE.md) for more information.
...
-   query
    ...
    A [SELECT](SELECT.md) or [VALUES](VALUES.md) command. This query will run within a security-restricted operation; in particular, calls to functions that themselves create temporary tables will fail.
