...
You can also use `gplogfilter` to search through all segment log files at once by running it through the [gpssh](gpssh.md) utility. For example, to display the last three lines of each segment log file:
...
Using [gpssh](gpssh.md), run `gplogfilter` on the segment hosts and search for log messages in the segment log files containing the string `con6` and save output to a file.
