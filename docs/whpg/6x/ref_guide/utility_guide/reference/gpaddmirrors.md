...
When enabling a mirroring configuration that adds hosts to the WarehousePG cluster, `gpaddmirrors` populates the [gp_segment_configuration](../../system_catalogs/system_catalogs_definitions/gp_segment_configuration.md) catalog table with the mirror segment instance information. WarehousePG uses the address value of the `gp_segment_configuration` catalog table when looking up host systems for WarehousePG interconnect (internal) communication between the coordinator and segment instances and between segment instances, and for other internal communication.
...
[gpinitsystem](gpinitsystem.md), [gpinitstandby](gpinitstandby.md), [gpactivatestandby](gpactivatestandby.md)
