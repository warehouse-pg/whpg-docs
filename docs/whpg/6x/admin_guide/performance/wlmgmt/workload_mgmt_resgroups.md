...
You can set the server configuration parameter [gp_resource_group_bypass](../../../ref_guide/config_params/guc-list.md) to bypass a resource group concurrency limit.

You can set the server configuration parameter [gp_resource_group_queuing_timeout](../../../ref_guide/config_params/guc-list.md) to specify the amount of time a transaction remains in the queue before WarehousePG cancels the transaction. The default timeout is zero, WarehousePG queues transactions indefinitely.
...
The [gp_resource_group_memory_limit](../../../ref_guide/config_params/guc-list.md) server configuration parameter identifies the maximum percentage of system memory resources to allocate to resource groups on each WarehousePG segment host. The default `gp_resource_group_memory_limit` value is .7 (70%).
...
When `MEMORY_SPILL_RATIO` is 0, WarehousePG uses the [`statement_mem`](../../../ref_guide/config_params/guc-list.md) server configuration parameter value to control initial query operator memory.

> **Note** When you set `MEMORY_LIMIT` to 0, `MEMORY_SPILL_RATIO` must also be set to 0.

You can selectively set the `MEMORY_SPILL_RATIO` on a per-query basis at the session level with the [memory_spill_ratio](../../../ref_guide/config_params/guc-list.md) server configuration parameter.
...
-   The resource group functions similarly to a resource queue, using the [`statement_mem`](../../../ref_guide/config_params/guc-list.md) server configuration parameter value to control initial query operator memory.
...
When you install WarehousePG, resource queues are enabled by default. To use resource groups instead of resource queues, you must set the [gp_resource_manager](../../../ref_guide/config_params/guc-list.md) server configuration parameter.
...
The [DROP RESOURCE GROUP](../../../ref_guide/sql_commands/DROP_RESOURCE_GROUP.md) command drops a resource group. To drop a resource group for a role, the group cannot be assigned to any role, nor can there be any transactions active or waiting in the resource group. Dropping a resource group for an external component in which there are running instances terminates the running instances.
...
The [gp_resgroup_config](../../../ref_guide/system_catalogs/system_catalogs_definitions/gp_resgroup_config.md) `gp_toolkit` system view displays the current limits for a resource group. To view the limits of all resource groups:
...
The [gp_resgroup_status](../../../ref_guide/gp_toolkit.md) `gp_toolkit` system view enables you to view the status and activity of a resource group. The view displays the number of running and queued transactions. It also displays the real-time CPU and memory usage of the resource group. To view this information:
...
The [gp_resgroup_status_per_host](../../../ref_guide/gp_toolkit.md) `gp_toolkit` system view enables you to view the real-time CPU and memory usage of a resource group on a per-host basis. To view this information:
...
The [gp_resgroup_status_per_segment](../../../ref_guide/gp_toolkit.md) `gp_toolkit` system view enables you to view the real-time CPU and memory usage of a resource group on a per-segment, per-host basis. To view this information:
...
To view a resource group's running queries, pending queries, and how long the pending queries have been queued, examine the [pg_stat_activity](../../../ref_guide/system_catalogs/system_catalogs_definitions/pg_stat_activity.md) system catalog table:
