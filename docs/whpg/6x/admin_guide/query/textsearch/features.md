...
[Parsing Documents](controlling.md#parsing-documents) showed how raw textual documents can be converted into `tsvector` values. WarehousePG also provides functions and operators that can be used to manipulate documents that are already in `tsvector` form.
...
[Parsing Queries](controlling.md#parsing-queries) showed how raw textual queries can be converted into `tsquery` values. WarehousePG also provides functions and operators that can be used to manipulate queries that are already in `tsquery` form.
