...
By default, the `RECURSIVE` keyword for the `WITH` clause is enabled. `RECURSIVE` can be deactivated by setting the server configuration parameter [gp_recursive_cte](../../ref_guide/config_params/guc-list.md) to `false`.
