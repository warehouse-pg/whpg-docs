...
When a user connects to the database via the WarehousePG coordinator and issues a query, processes are created in each segment database to handle the work of that query. For more information about query processes, see [About WarehousePG Query Processing](../query/parallel-proc.md).
