...
3.  Use the [gpscp](../../../ref_guide/utility_guide/reference/gpscp.md) utility and the `hosts_all` file to copy the `tar.gz` files to the same directory on all nodes of the WarehousePG cluster. The `hosts_all` file contains a list of all the WarehousePG segment hosts. You might require root access to do this.
