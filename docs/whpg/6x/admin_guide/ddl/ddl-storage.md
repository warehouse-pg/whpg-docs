...
The `ALTER TABLE`command changes the definition of a table. Use `ALTER TABLE` to change table attributes such as column definitions, distribution policy, storage model, and partition structure (see also [Maintaining Partitioned Tables](ddl-partition/index.md)). For example, to add a not-null constraint to a table column:
