...
description: Configure PXF to read and write data on a network file system, and query it from WarehousePG.
---
...
1. Uncomment `pxf.fs.basePath` and set it to the mount point, and set `pxf.service.user.impersonation` to `false`, since this connector always accesses files as the OS user running PXF rather than the connecting WHPG user:
    ...
    The path in `LOCATION` is relative to `pxf.fs.basePath`. See [Configuration templates](../reference/configuration-templates.md#pxf-sitexml) for the full list of `pxf-site.xml` properties.
...
The path in `LOCATION` can't be relative, and can't include the `$` character. See [PXF profiles](../reference/profiles.md) for the full list of supported formats, including worked examples of Avro, JSON, and multi-byte delimiters.
