...
description: Configure PXF to read and write data in an external SQL database over JDBC, and query it from WarehousePG.
---
...
1. Edit `jdbc-site.xml` with your connection details. For example, to connect to a WHPG database named `gpadmin` using the bundled PostgreSQL JDBC driver:
    ...
    `jdbc-site.xml` also has commented-out properties for connection pooling, batch and fetch sizes, session and connection-level settings, and user impersonation. Uncomment and set only the ones you need. See [Configuration templates](../reference/configuration-templates.md#jdbc-sitexml) for the full list.
