...
This page covers PXF for WHPG 7.x. See [PXF for WHPG 6.x](../../6x/overview/compatibility.md) for that version instead.
...
::: info Note
[PXF foreign data wrapper](../foreign-data-wrapper.md) is available starting with WHPG 7.x. It isn't available for WHPG 6.x.
:::
...
| HBase | HBase 2.6.x, coordinated with ZooKeeper 3.8.x |
| Amazon S3 and other S3-compatible object stores, including MinIO | S3 API, through the AWS SDK for Java 1.12.x |
| Azure Blob Storage and Azure Data Lake Storage Gen2 | Bundled with PXF's Hadoop 3.3.x client |
