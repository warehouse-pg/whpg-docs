...
- [gpbackup syntax reference](reference/gpbackup.md)
- [gprestore syntax reference](reference/gprestore.md)
:::
...
1. Build the `gpbackup_s3_plugin` binary:
...
1. With the WarehousePG environment sourced, install the binary on all segment hosts:
...
1. Verify that the binary is present and executable on every host, for example using `gpssh` with the `all_hosts` file described in [Installing WarehousePG Backup and Restore](installing.md):
