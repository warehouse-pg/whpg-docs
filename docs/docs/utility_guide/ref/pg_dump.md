...
`pg_dump` is a standard PostgreSQL utility for backing up a database, and is also supported in WarehousePG Database. It creates a single \(non-parallel\) dump file. For routine backups of WarehousePG Database, it is better to use the WarehousePG Database backup utility, [gpbackup](https://docs.vmware.com/en/VMware-WarehousePG-Backup-and-Restore/index.html), for the best performance.

Use `pg_dump` if you are migrating your data to another database vendor's system, or to another WarehousePG Database system with a different segment configuration \(for example, if the system you are migrating to has greater or fewer segment instances\). To restore, you must use the corresponding [pg\_restore](pg_restore.html) utility \(if the dump file is in archive format\), or you can use a client program such as [psql](psql.html) \(if the dump file is in plain text format\).

Since `pg_dump` is compatible with regular PostgreSQL, it can be used to migrate data into WarehousePG Database. The `pg_dump` utility in WarehousePG Database is very similar to the PostgreSQL `pg_dump` utility, with the following exceptions and limitations:

-   If using `pg_dump` to backup a WarehousePG Database database, keep in mind that the dump operation can take a long time \(several hours\) for very large databases. Also, you must make sure you have sufficient disk space to create the dump file.
-   If you are migrating data from one WarehousePG Database system to another, use the `--gp-syntax` command-line option to include the `DISTRIBUTED BY` clause in `CREATE TABLE` statements. This ensures that WarehousePG Database table data is distributed with the correct distribution key columns upon restore.
...
:   This option allows running `pg_dump -j` against a pre-6.0 WarehousePG Database server; see the documentation of the `-j` parameter for more details.
...
:   > **Note** Because WarehousePG Database does not support serializable transactions, the `--serializable-deferrable` option has no effect in WarehousePG Database.
...
:   Use `--gp-syntax` to dump WarehousePG Database syntax in the `CREATE TABLE` statements. This allows the distribution policy \(`DISTRIBUTED BY` or `DISTRIBUTED RANDOMLY` clauses\) of a WarehousePG Database table to be dumped, which is useful for restoring into other WarehousePG Database systems. The default is to include WarehousePG Database syntax when connected to a WarehousePG Database system, and to exclude it when connected to a regular PostgreSQL system.
...
-h \<host\> \| --host=\<host\>
:   The host name of the machine on which the WarehousePG Database coordinator database server is running. If not specified, reads from the environment variable `PGHOST` or defaults to `localhost`.
...
Because `pg_dump` may be used to transfer data to newer versions of WarehousePG Database, the output of `pg_dump` can be expected to load into WarehousePG Database versions newer than `pg_dump`'s version. `pg_dump` can also dump from WarehousePG Database versions older than its own version. However, `pg_dump` cannot dump from WarehousePG Database versions newer than its own major version; it will refuse to even try, rather than risk making an invalid dump. Also, it is not guaranteed that `pg_dump`'s output can be loaded into a server of an older major version — not even if the dump was taken from a server of that version. Loading a dump file into an older server may require manual editing of the dump file to remove syntax not understood by the older server. Use of the `--quote-all-identifiers` option is recommended in cross-version cases, as it can prevent problems arising from varying reserved-word lists in different WarehousePG Database versions.
...
Dump a WarehousePG Database in tar file format and include distribution policy information:
