...
Extracts all databases in a WarehousePG Database system to a single script file or other archive file.
...
`pg_dumpall` needs to connect several times to the WarehousePG Database coordinator server \(once per database\). If you use password authentication it is likely to ask for a password each time. It is convenient to have a `~/.pgpass` file in such cases.
...
--gp-syntax
:   Output WarehousePG Database syntax in the `CREATE TABLE` statements. This allows the distribution policy \(`DISTRIBUTED BY` or `DISTRIBUTED RANDOMLY` clauses\) of a WarehousePG Database table to be dumped, which is useful for restoring into other WarehousePG Database systems.
