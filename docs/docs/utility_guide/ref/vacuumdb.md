...
`vacuumdb` is a utility for cleaning a WarehousePG Database database. `vacuumdb` will also generate internal statistics used by the WarehousePG Database query optimizer.
...
:   Selects a full vacuum, which may reclaim more space, but takes much longer and exclusively locks the table.
    ...
    > **Caution** A `VACUUM FULL` is not recommended in WarehousePG Database.
