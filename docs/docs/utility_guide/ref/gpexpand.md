...
Expands an existing WarehousePG Database across new hosts in the system.
...
-   You are logged in as the WarehousePG Database superuser \(`gpadmin`\).
...
-   When redistributing data, WarehousePG Database must be running in production mode. WarehousePG Database cannot be running in restricted mode or in coordinator mode. The `gpstart` options `-R` or `-m` cannot be specified to start WarehousePG Database.
...
> **Important** When expanding a WarehousePG Database system, you must deactivate WarehousePG interconnect proxies before adding new hosts and segment instances to the system, and you must update the `gp_interconnect_proxy_addresses` parameter with the newly-added segment instances before you re-enable interconnect proxies. For information about WarehousePG interconnect proxies, see [Configuring Proxies for the WarehousePG Interconnect](../../admin_guide/managing/proxy-ic.html).

For information about preparing a system for expansion, see [Expanding a WarehousePG System](../../admin_guide/expand/expand-main.html)in the *WarehousePG Database Administrator Guide*.
...
-S \| --simple-progress
:   If specified, the `gpexpand` utility records only the minimum progress information in the WarehousePG Database table *gpexpand.expansion\_progress*. The utility does not record the relation size information and status information in the table *gpexpand.status\_detail*.
...
\[-t \| --tardir\] directory
:   The fully qualified path to a directory on segment hosts where the `gpexpand` utility copies a temporary tar file. The file contains WarehousePG Database files that are used to create segment instances. The default directory is the user home directory.
...
When expanding a WarehousePG Database system, you can specify either a hostname or an IP address for the value.
...
-   If you specify an IP address, the address should not be changed after the initial configuration. When segment mirroring is enabled, replication from the primary to the mirror segment will fail if the IP address changes from the configured value. For this reason, you should use a hostname when expanding a WarehousePG Database system unless you have a specific requirement to use IP addresses.

When expanding a WarehousePG system, `gpexpand` populates [gp\_segment\_configuration](../../ref_guide/system_catalogs/gp_segment_configuration.html) catalog table with the new segment instance information. WarehousePG Database uses the `address` value of the `gp_segment_configuration` catalog table when looking up host systems for WarehousePG interconnect \(internal\) communication between the coordinator and segment instances and between segment instances, and for other internal communication.

## <a id="multi_nic"></a>Using Host Systems with Multiple NICs 

If host systems are configured with multiple NICs, you can expand a WarehousePG Database system to use each NIC as a WarehousePG host system. You must ensure that the host systems are configured with sufficient resources to support all the segment instances being added to the host. Also, if you enable segment mirroring, you must ensure that the expanded WarehousePG system configuration supports failover if a host system fails. For information about WarehousePG Database mirroring schemes, see [../../best\_practices/ha.html\#topic\_ngz\_qf4\_tt](../../best_practices/ha.html#topic_ngz_qf4_tt).

For example, this is a `gpexpand` configuration file for a simple WarehousePG system. The segment host `gp7s1` and `gp7s2` are configured with two NICs, `-s1` and `-s2`, where the WarehousePG Database system uses each NIC as a host system.
...
[gpssh-exkeys](gpssh-exkeys.html), [Expanding a WarehousePG System](../../admin_guide/expand/expand-main.html)
