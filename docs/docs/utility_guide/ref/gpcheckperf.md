...
-B block\_size
:   Specifies the block size \(in KB or MB\) to use for disk I/O test. The default is 32KB, which is the same as the WarehousePG Database page size. The maximum block size is 1 MB.
