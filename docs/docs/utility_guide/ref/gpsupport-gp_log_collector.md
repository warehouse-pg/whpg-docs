...
This tool collects WarehousePG and system log files, along with the relevant configuration parameters, and generates a file which can be provided to VMware Customer Support for diagnosis of errors or system failures.
...
-skip-coordinator
:   When running `gp_log_collector`, the generated tarball can be very large. Use this option to skip WarehousePG Coordinator log collection when only WarehousePG Segment logs are required.

-with-gpbackup 
:   Beginning with WarehousePG 6.22, this option enables you to collect logs related to backup and restore. 
...
-with-gptext
:   Collect all GPText logs along with WarehousePG logs.

-with-gptext-only
:   Collect only GPText logs.

-with-gpss 
:  Collect log files related to WarehousePG Streaming Server. If you do not specify a directory with the `-gpsslogdir` option, gpsupport collects logs from the `gpAdminLogs` directory. Log files are of the format `gpss_<date>.log`.

-with-pxf
:   Collect all PXF logs along with WarehousePG logs.

-with-pxf-only
:   Collect only PXF logs.

-with-gpupgrade
:   Collect all `gpupgrade` logs along with WarehousePG logs.

-with-gpdr-primary
:   Collect all logs relevant for a WarehousePG Disaster Recovery primary cluster, along with WarehousePG logs.

-with-gpdr-recovery
: Collect all logs relevant for a WarehousePG Disaster Recovery recovery cluster, along with WarehousePG logs.
...
| WarehousePG Disaster Recovery files | <ul><li>GPDB pg_logs</li><li>`gpdr_YYYYMMDD.log` files in gpAdminLogs</li><li>GPDR config files in `$GPDR_HOME/configs/`</li><li>GPDR pgbackrest log files in `$GPDR_HOME/logs/`</li><li>GPDR state files in `$GPDR_HOME/state/`</li></ul> |
...
Collect WarehousePG coordinator and segment logs listed in a hostfile from today:
...
Collect only GPText logs for all segments, without any WarehousePG logs:
