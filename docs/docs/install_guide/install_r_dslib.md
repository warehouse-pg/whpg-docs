...
R packages are modules that contain R functions and data sets. WarehousePG Database provides a collection of data science-related R libraries that can be used with the WarehousePG Database PL/R language. You can download these libraries in `.gppkg` format from [VMware Tanzu Network](https://network.pivotal.io/products/pivotal-gpdb).
...
For information about the WarehousePG Database PL/R Language, see [WarehousePG PL/R Language Extension](../analytics/pl_r.html).
...
Before you install the R Data Science Library package, make sure that your WarehousePG Database is running, you have sourced `greenplum_path.sh`, and that the `COORDINATOR_DATA_DIRECTORY` and `$GPHOME` environment variables are set.
...
3.  Follow the instructions in [Verifying the WarehousePG Database Software Download](../install_guide/verify_sw.html) to verify the integrity of the *WarehousePG Procedural Languages R Data Science Package* software.
3.  Use the `gppkg` command to install the package. For example:
    ...
    `gppkg` installs the R Data Science libraries on all nodes in your WarehousePG Database cluster. The command also sets the `R_LIBS_USER` environment variable and updates the `PATH` and `LD_LIBRARY_PATH` environment variables in your `greenplum_path.sh` file.

4.  Restart WarehousePG Database. You must re-source `greenplum_path.sh` before restarting your WarehousePG cluster:
...
The WarehousePG Database R Data Science Modules are installed in the following directory:
...
The command removes the R Data Science libraries from your WarehousePG Database cluster. It also removes the `R_LIBS_USER` environment variable and updates the `PATH` and `LD_LIBRARY_PATH` environment variables in your `greenplum_path.sh` file to their pre-installation values.

Re-source `greenplum_path.sh` and restart WarehousePG Database after you remove the R Data Science Library package:
...
> **Note** When you uninstall the R Data Science Library package from your WarehousePG Database cluster, any UDFs that you have created that use R libraries installed with this package will return an error.
