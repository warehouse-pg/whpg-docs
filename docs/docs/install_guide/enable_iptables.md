...
> **Note** WarehousePG Database performance might be impacted when `iptables` is enabled. You should test the performance of your application with `iptables` enabled to ensure that performance is acceptable.
...
1.  As `gpadmin`, run this command on the WarehousePG Database coordinator host to stop WarehousePG Database:

    ```
    $ gpstop -a
    ```

2.  On the WarehousePG Database hosts:
...
3.  As gpadmin, run this command on the WarehousePG Database coordinator host to start WarehousePG Database:
...
To ensure that the WarehousePG Database workload does not overflow the `iptables` table, as root, set it to the following value:
...
When `iptables` is enabled, `iptables` manages the IP communication on the host system based on configuration settings \(rules\). The example rules are used to configure `iptables` for WarehousePG Database coordinator host, standby coordinator host, and segment hosts.

-   [Example Coordinator and Standby Coordinator iptables Rules](#topic17)
-   [Example Segment Host iptables Rules](#topic18)

The two sets of rules account for the different types of communication WarehousePG Database expects on the coordinator \(primary and standby\) and segment hosts. The rules should be added to the `/etc/sysconfig/iptables` file of the WarehousePG Database hosts. For WarehousePG Database, `iptables` rules should allow the following communication:

-   For customer facing communication with the WarehousePG Database coordinator, allow at least `postgres` and `28080` \(`eth1` interface in the example\).
-   For WarehousePG Database system interconnect, allow communication using `tcp`, `udp`, and `icmp` protocols \(`eth4` and `eth5` interfaces in the example\).

    The network interfaces that you specify in the `iptables` settings are the interfaces for the WarehousePG Database hosts that you list in the hostfile\_gpinitsystem file. You specify the file when you run the `gpinitsystem` command to initialize a WarehousePG Database system. See [Initializing WarehousePG](init_whpg.html) for information about the hostfile\_gpinitsystem file and the `gpinitsystem` command.
...
Example `iptables` rules with comments for the `/etc/sysconfig/iptables` file on the WarehousePG Database coordinator host and standby coordinator host.
...
# and WarehousePG Database uses random udp ports in the range 1025 to 65535.
...
# Allow WarehousePG Database on all networks.
-A INPUT -p tcp --dport postgres -j ACCEPT --syn -m state --state NEW
# Allow WarehousePG Command Center on the customer facing network.
...
Example `iptables` rules for the `/etc/sysconfig/iptables` file on the WarehousePG Database segment hosts. The rules for segment hosts are similar to the coordinator rules with fewer interfaces and fewer `udp` and `tcp` services.
