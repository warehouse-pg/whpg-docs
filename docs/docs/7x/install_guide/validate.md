...
WarehousePG provides a management utility called [gpcheckperf](../ref_guide/utility_guide/reference/gpcheckperf.md), which can be used to identify hardware and system-level issues on the machines in your WarehousePG cluster. [gpcheckperf](../ref_guide/utility_guide/reference/gpcheckperf.md) starts a session on the specified hosts and runs the following performance tests:

-   Network Performance (`gpnetbench*`)
-   Disk I/O Performance (`dd` test)
-   Memory Bandwidth (`stream` test)

Before using `gpcheckperf`, you must have a trusted host setup between the hosts involved in the performance test. You can use the utility [gpssh-exkeys](../ref_guide/utility_guide/reference/gpssh-exkeys.md) to update the known host files and exchange public keys between hosts if you have not done so already. Note that `gpcheckperf` calls to [gpssh](../ref_guide/utility_guide/reference/gpssh.md) and [gpsync](../ref_guide/utility_guide/reference/gpsync.md), so these WarehousePG utilities must be in your `$PATH`.

-   **[Validating Network Performance](validate.md)**  

-   **[Validating Disk I/O and Memory Bandwidth](validate.md)**  
...
You would create four distinct host files for use with the [gpcheckperf](../ref_guide/utility_guide/reference/gpcheckperf.md) network test:
...
You would then run [gpcheckperf](../ref_guide/utility_guide/reference/gpcheckperf.md) once per subnet. For example (if testing an *even* number of hosts, run in parallel pairs test mode):
