...
-   The last step of the software phase is redistributing table data. Using the expansion control tables in the *gpexpand* schema as a guide, tables are redistributed. For each table:
    ...
    -   The [gpexand](../../../ref_guide/utility_guide/reference/gpexpand.md) utility redistributes the table data, across all of the servers, old and new, according to the distribution policy.
...
1.  To [create an expansion input file](expand-initialize.md):

    ```
    gpexpand -f <hosts_file>
    ```

2.  To [initialize segments and create the expansion schema](expand-initialize.md):
...
3.  [To redistribute table data](expand-redistribute.md):
    ...
    You may need to run `gpexpand` several times to complete the expansion in large-scale systems that require multiple redistribution sessions. `gpexpand` can benefit from explicit table redistribution ranking; see [Planning Table Redistribution](expand-planning.md).
...
For information about the [gpexpand](../../../ref_guide/utility_guide/reference/gpexpand.md) utility and the other utilities that are used for system expansion, see the *WarehousePG Utility Guide*.
