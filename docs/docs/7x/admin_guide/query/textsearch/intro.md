...
WarehousePG provides a data type `tsvector` to store preprocessed documents, and a data type `tsquery` to store processed queries \([Text Search Data Types](../../ref_guide/datatype-textsearch.html)\). There are many functions and operators available for these data types \([Text Search Functions and Operators](../../ref_guide/function-summary.html)\), the most important of which is the match operator `@@`, which we introduce in [Basic Text Matching](#basic-text-matching). Full text searches can be accelerated using indexes \([GiST and GIN Indexes for Text Search](gist-gin.html)\).
