...
You partition a table when you create it with the [CREATE TABLE](../../../ref_guide/sql_commands/CREATE_TABLE.md) command. Before you create the table, you should:
...
> **Important** The examples in the next section use the modern partitioning syntax. Refer to [Examples Using the Classic Partitioning Syntax](classic-partition.md) for information about the classic partitioning syntax and parallel examples.
...
When a table is partitioned based on the query predicate, you can use [EXPLAIN](../../../ref_guide/sql_commands/EXPLAIN.md) to verify that the query optimizer scans only the relevant data to examine the query plan.
...
Partitioning information is stored in the [pg_partitioned_table](../../../ref_guide/system_catalogs/system_catalogs_definitions/pg_partitioned_table.md) system catalog, and in additional fields in the [pg_class](../../../ref_guide/system_catalogs/system_catalogs_definitions/pg_class.md) (`relispartition` and `relpartbound`) catalog.
...
For a partitioned table that you create using the classic syntax, the [gp_partition_template](../../../ref_guide/system_catalogs/system_catalogs_definitions/gp_partition_template.md) system catalog table describes the relationship between the partitioned table and the sub-partition template defined at each level in the partition hierarchy.
...
Refer to [Migrating Partition Maintenance Scripts to the New WarehousePG 7 Partitioning Catalogs](../../../install_guide/migrate-classic-partitioning.md) for information about mapping WarehousePG 6 partitioning catalogs to the new definitions in WarehousePG 7.
...
You can enable or disable partition pruning for the Postgres-based planner by setting the [enable_partition_pruning](../../../ref_guide/config_params/guc-list.md#enable_partition_pruning) server configuration parameter.
...
> **Important** The examples in the next section use the modern partitioning syntax. Refer to [Partition Maintenance with the Classic Syntax](classic-partition.md#topic77c) for information about the maintenance activities and commands for partitioned tables that you created using the classic partitioning syntax.
