...
Make sure that your server keytab file is readable (and preferably only readable) by the `gpadmin` server account. The location of the key file is specified by the [krb_server_keyfile](../ref_guide/config_params/guc-list.md) configuration parameter. For security reasons, it is recommended to use a separate keytab just for the WarehousePG server rather than opening up permissions on the system keytab file.
...
> **Note** For information about encrypting data between the `gpfdist` server and WarehousePG segment hosts, see [Encrypting gpfdist Connections](encryption.md).
