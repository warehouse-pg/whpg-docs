...
XFS is the file system used for WarehousePG data directories. Use the mount options described in [Configure Operating System](../install_guide/config_os.md).

<a id="port_config"></a>

## Port Configuration

See the [recommended OS parameter settings](../install_guide/config_os.md#topic3) in the *WarehousePG Installation Guide* for further details.
...
See the [Recommended OS Parameters Settings](../install_guide/config_os.md#topic3) in the *WarehousePG Installation Guide* for further details.
...
The Linux sysctl `vm.overcommit_memory` and `vm.overcommit_ratio` variables affect how the operating system manages memory allocation. See the [`/etc/sysctl.conf`](../install_guide/config_os.md#sysctl_file) file parameters guidelines in the *WarehousePG Datatabase Installation Guide* for further details.
...
See also [Memory and Resource Management with Resource Queues](workloads.md).

<a id="shared_mem_config"></a>

## Shared Memory Settings

WarehousePG uses shared memory to communicate between `postgres` processes that are part of the same `postgres` instance. The following shared memory settings should be set in `sysctl` and are rarely modified. See the [`sysctl.conf`](../install_guide/config_os.md) file parameters in the *WarehousePG Installation Guide* for further details.
...
See [Setting the WarehousePG Recommended OS Parameters](../install_guide/config_os.md#recommended-os-parameters-settings) for more details.
...
Also, see [Resource Management](workloads.md) for best practices for managing query memory using resource queues.
...
-   The `gp_workfile_entries` view contains one row for each operator using disk space for workfiles on a segment at the current time. See [How to Read Explain Plans](tuning_queries.md) for information about operators.
