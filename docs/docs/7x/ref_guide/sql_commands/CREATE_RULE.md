...
> **Note** A view that is simple enough to be automatically updatable (see [CREATE VIEW](CREATE_VIEW.md)) does not require a user-created rule in order to be updatable. While you can create an explicit rule anyway, the automatic update transformation will generally outperform an explicit rule.
...
[ALTER RULE](ALTER_RULE.md), [DROP RULE](DROP_RULE.md)
