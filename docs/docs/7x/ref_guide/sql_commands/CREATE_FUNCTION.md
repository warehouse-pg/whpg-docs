...
If you drop and then recreate a function, the new function is not the same entity as the old; you will have to drop existing objects (rules, views, triggers, and so on) that refer to the old function. Use `CREATE OR REPLACE FUNCTION` to change a function definition without breaking objects that refer to the function. Also, [ALTER FUNCTION](ALTER_FUNCTION.md) can be used to change most of the auxiliary properties of an existing function.
...
See [Using Functions and Operators](../../admin_guide/query/functions-operators.md) in the *WarehousePG Administrator Guide*.
...
See [SET](SET.md) for more information about allowed parameter names and values.
...
This form of the `AS` clause is used for dynamically loadable C language functions when the function name in the C language source code is not the same as the name of the SQL function. The string obj_file is the name of the file containing the dynamically loadable object, and is interpreted as for the [LOAD](LOAD.md) command. The string link_symbol is the name of the function in the C language source code. If the link symbol is omitted, it is assumed to be the same as the name of the SQL function being defined. The C names of all functions must be different, so you must give overloaded SQL functions different C names (for example, use the argument types as part of the C names).
...
Another point to keep in mind is that by default, execute privilege is granted to `PUBLIC` for newly created functions (see [GRANT](GRANT.md) for more information). Frequently you will wish to restrict use of a security definer function to only some users. To do that, you must revoke the default `PUBLIC` privileges and then grant `EXECUTE` privilege selectively. To avoid having a window where the new function is accessible to all, create it and set the privileges within a single transaction. For example:
...
[ALTER FUNCTION](ALTER_FUNCTION.md), [DROP FUNCTION](DROP_FUNCTION.md), [GRANT](GRANT.md), [LOAD](LOAD.md), [REVOKE](REVOKE.md), [createlang](../utility_guide/reference/createdb.md)
