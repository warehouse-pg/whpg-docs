...
There is no `LOCK TABLE` in the SQL standard, which instead uses `SET TRANSACTION` to specify concurrency levels on transactions. WarehousePG supports that too; see [SET TRANSACTION](SET_TRANSACTION.html) for details.
