...
with_query
The `WITH` clause allows you to specify one or more subqueries that can be referenced by name in the `INSERT` query.  See [WITH Queries (Common Table Expressions)](../../admin_guide/query/CTE-query.md) and [SELECT](SELECT.md) for details.
...
Possible limitations of the query clause are documented under [SELECT](SELECT.md).
