...
INLINE inline_handler
The name of a previously registered function that is called to run an anonymous code block in this language that is created with the [DO](DO.md) command. If no inline_handler function is specified, the language does not support anonymous code blocks. The handler function must take one argument of type `internal`, which is the [DO](DO.md) command internal representation. The function typically returns `void`. The return value of the handler is ignored.
...
Use [DROP LANGUAGE](DROP_LANGUAGE.md) to drop procedural languages.

The system catalog [pg_language](../system_catalogs/system_catalogs_definitions/pg_language.md) records information about the currently installed languages. Also, the [psql](../utility_guide/reference/psql.md) command `\dL` lists the installed languages.
...
[ALTER LANGUAGE](ALTER_LANGUAGE.md), [CREATE EXTENSION](CREATE_EXTENSION.md), [CREATE FUNCTION](CREATE_FUNCTION.md), [DROP LANGUAGE](DROP_LANGUAGE.md), [GRANT](GRANT.md), [DO](DO.md)
