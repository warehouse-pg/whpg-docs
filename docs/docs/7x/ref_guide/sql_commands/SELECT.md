...
> **IMPORTANT**
> To call the `SYSTEM_ROWS` and `SYSTEM_TIME` sampling methods, you must install the WarehousePG extensions that provide access to them: `tsm_system_rows` and `tsm_system_time`, repectively. See the [tsm_system_rows](./../modules/bundled/tsm_system_rows.md) and [tsm_system_time](./../modules/bundled/tsm_system_time.md) topics for details.
