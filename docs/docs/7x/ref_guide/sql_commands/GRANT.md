...
The [REVOKE](REVOKE.md) command is used to revoke access privileges.
...
The `GRANT` command cannot be used to set privileges for the protocols `file`, `gpfdist`, or `gpfdists`. These protocols are implemented internally in WarehousePG. Instead, use the [CREATE ROLE](CREATE_ROLE.md) or [ALTER ROLE](ALTER_ROLE.md) command to set the `CREATEEXTTABLE` attribute for the role.
...
[ALTER DEFAULT PRIVILEGES](ALTER_DEFAULT_PRIVILEGES.md), [REVOKE](REVOKE.md)
