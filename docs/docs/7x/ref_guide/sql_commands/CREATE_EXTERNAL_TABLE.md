...
See [Working with External Data](../../admin_guide/external/index.md) for detailed information about working with external tables.
...
Character set encoding to use for the external table. Specify a string constant (such as `'SQL_ASCII'`), an integer encoding number, or `DEFAULT` to use the default server encoding. See [Character Set Support](../character_sets.md).
...
You can change the limit for the number of initial rejected rows with the WarehousePG server configuration parameter [gp_initial_bad_row_limit](../config_params/guc-list.md#gp_initial_bad_row_limit).
...
When you specify the `LOG ERRORS` clause, WarehousePG captures errors that occur while reading the external table data. For information about the error log format, see [Viewing Bad Rows in the Error Log](../../admin_guide/load/handling-load-errors/viewing-bad-rows-in-the-error-table-or-error-log.md).
...
[CREATE TABLE AS](CREATE_TABLE_AS.md), [CREATE TABLE](CREATE_TABLE.md), [COPY](COPY.md), [SELECT INTO](SELECT_INTO.md), [INSERT](INSERT.md)
