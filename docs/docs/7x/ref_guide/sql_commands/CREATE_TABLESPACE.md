...
A user with appropriate privileges can pass tablespace_name to [CREATE DATABASE](CREATE_DATABASE.md), [CREATE TABLE](CREATE_TABLE.md), or [CREATE INDEX](CREATE_INDEX.md) to direct WarehousePG to store the data files for these objects within the specified tablespace.
