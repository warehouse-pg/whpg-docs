...
table_name
The name (optionally schema-qualified) of the table containing the column(s) on which the statistics are computed; see [ANALYZE](ANALYZE.md) for an explanation of inheritance and partition handling.
...
[ALTER STATISTICS](ALTER_STATISTICS.md), [DROP STATISTICS](DROP_STATISTICS.md)
