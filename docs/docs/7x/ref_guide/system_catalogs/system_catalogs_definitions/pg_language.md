...
The `pg_language` system catalog table registers languages in which you can write functions or stored procedures. See [CREATE LANGUAGE](../../sql_commands/CREATE_LANGUAGE.md) for more information about language handlers.
...
| `laninline`     | oid        | pg_proc.oid   | This references a function that is responsible for running inline anonymous code blocks (see the [DO](../../sql_commands/DO.md) command). Zero if inline blocks are not supported.                                                            |
