...
| `opcmethod`    | oid     | [pg_am](pg_am.md).oid               | Index access method operator class is for                                |
| `opcname`      | name    |                                     | Name of this operator class                                              |
| `opcnamespace` | oid     | [pg_namespace](pg_namespace.md).oid | Namespace of this operator class                                         |
| `opcowner`     | oid     | [pg_authid](pg_authid.md).oid       | Owner of the operator class                                              |
| `opcfamily`    | oid     | [pg_opfamily](pg_opfamily.md).oid   | Operator family containing the operator class                            |
| `opcintype`    | oid     | [pg_type](pg_type.md).oid           | Data type that the operator class indexes                                |
| `opcdefault`   | boolean |                                     | True if this operator class is the default for the data type `opcintype` |
| `opckeytype`   | oid     | [pg_type](pg_type.md).oid           | Type of data stored in index, or zero if same as `opcintype`             |
