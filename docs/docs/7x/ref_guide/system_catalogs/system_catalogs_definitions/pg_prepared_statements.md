...
The `pg_prepared_statements` view displays all of the prepared statements that are available in the current session. See [PREPARE](../../sql_commands/PREPARE.md) for more information about prepared statements.

`pg_prepared_statements` contains one row for each prepared statement. Rows are added to the view when a new prepared statement is created and removed when a prepared statement is released (for example, with the [DEALLOCATE](../../sql_commands/DEALLOCATE.md) command).
