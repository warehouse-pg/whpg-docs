...
-   pg_resqueue_status (Deprecated. Use [gp_toolkit.gp_resqueue_status](../gp_toolkit.md).)
...
-   session_level_memory_consumption (See [Monitoring a WarehousePG cluster](../../admin_guide/managing/monitor.md#topic_slt_ddv_1q).)
...
For more information on handling suboverflowed backends to prevent performance issues, see [Checking for and Terminating Overflowed Backends](../../admin_guide/managing/monitor.md#overflowed_backends).
...
Status for specific tables involved in the expansion is stored in [gpexpand.status_detail](system_catalogs_definitions/gp_expansion_tables.md).
...
| `phase`                     | text    | Current processing phase. Refer to [ANALYZE Progress Reporting](../../admin_guide/managing/progress_reporting.md#analyze_progress) for detailed information about the phases.                                                  |
...
| `phase`                | text    | Current processing phase. Refer to [Base Backup Progress Reporting](../../admin_guide/managing/progress_reporting.md#basebackup_progress) for detailed information about the phases.                                                                                                                                                                                                                                                                                      |
...
| `phase`               | text    | Current processing phase. Refer to [CLUSTER and VACUUM FULL Progress Reporting](../../admin_guide/managing/progress_reporting.md#cluster_progress) for detailed information about the phases.                                                                                                                                                                                                                                                                       |
...
| `phase`              | text    | Current processing phase of index creation. Refer to [CREATE INDEX Progress Reporting](../../admin_guide/managing/progress_reporting.md#create_index_progress) for detailed information about the phases. |
...
| `phase`              | text    | Current processing phase of vacuum. Refer to [VACUUM Progress Reporting](../../admin_guide/managing/progress_reporting.md#vacuum_progress) for detailed information about the phases.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                         |
...
| `max_dead_tuples`    | bigint  | *Heap tables*: Number of dead tuples that we can store before needing to perform an index vacuum cycle, based on [maintenance_work_mem](../config_params/guc-list.md#maintenance_work_mem). <br /><br />*AO/CO tables*: Collected at the beginning of the `append-optimized pre-cleanup` phase, this is the total number of tuples before the logical EOF of all segment files. The value should not change while `VACUUM` progresses.                                                                                                                                                                                                                                                                                                                                                                                                        |
...
| `wal_write_time`   | double precision         |            | Total amount of time spent writing WAL buffers to disk, in milliseconds (if [track_wal_io_timing](../config_params/guc-list.md#track_wal_io_timing) is enabled, otherwise zero). |
...
| ------------------------- | -------- | ------------------------------------------------------------------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `gp_segment_id`           | integer  |                                                                     | Unique identifier of a segment (or coordinator)                                                                                                                                                                                                                                                                                                                                                                                                                                                                             |
| `schemaname`              | name     | [pg_namespace](system_catalogs_definitions/pg_namespace.md).nspname | The name of the schema containing table.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                    |
| `tablename`               | name     | [pg_class](system_catalogs_definitions/pg_class.md).relname         | The name of the table.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                      |
| `attname`                 | name     | [pg_attribute](system_catalogs_definitions/pg_attribute.md).attname | The name of the column described by this row.                                                                                                                                                                                                                                                                                                                                                                                                                                                                               |
...
The maximum number of entries in the array fields can be controlled on a column-by-column basis using the `ALTER TABLE SET STATISTICS` command, or globally by setting the [default_statistics_target](../config_params/guc-list.md#default_statistics_target) run-time configuration parameter.
...
| `schemaname`             | name            | [pg_namespace](system_catalogs_definitions/pg_namespace.md).nspname         | The name of the schema containing table.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                      |
| `tablename`              | name            | [pg_class](system_catalogs_definitions/pg_class.md).relname                 | The name of the table.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                        |
| `statistics_schemaname`  | name            | [pg_namespace](system_catalogs_definitions/pg_namespace.md).nspname         | The name of the schema containing the extended statistic.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                     |
| `statistics_name`        | name            | [pg_statistic_ext](system_catalogs_definitions/pg_statistic_ext.md).stxname | The name of the extended statistic.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                           |
| `statistics_owner`       | oid             | [pg_authid](system_catalogs_definitions/pg_authid.md).oid                   | The owner of the extended statistic.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                          |
| `attnames`               | name\[]         | [pg_attribute](system_catalogs_definitions/pg_attribute.md).attname         | The names of the columns on which the extended statistics is defined.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                         |
