...
Prompts `gpfdist` to use SSL encryption for the data transfer. After running `gpfdist` with the `--ssl <certificate_path>` option, you must use the `gpfdists://` protocol to load data from this file server. For information on `gpfdists`, refer to [gpfdists:// Protocol](../../../admin_guide/external/external-tables/gpfdists-protocol.md) in the *WarehousePG Administrator Guide*.
...
The settings of both the [verify_gpfdists_cert](../../config_params/guc-list.md#verify_gpfdists_cert) server configuration parameter and the `--ssl_verify_peer` option also determine which certificate files are required on the WarehousePG segments as described in [gpfdists:// Protocol](../../../admin_guide/external/external-tables/gpfdists-protocol.md).
...
For information about the file format, see [Configuration File Format](../../../admin_guide/load/transforming-xml-data.md#topic83) in the *WarehousePG Administrator Guide*. For information about configuring data transformation with `gpfdist`, see [Transforming External Data with gpfdist and gpload](../../../admin_guide/load/transforming-xml-data.md#transforming-with-gpfdist-and-insert-into-select-from) in the *WarehousePG Administrator Guide*.
...
You can set the server configuration parameter [gpfdist_retry_timeout](../../config_params/guc-list.md) to control the time that WarehousePG waits before returning an error when a `gpfdist` server does not respond while WarehousePG is attempting to write data to `gpfdist`. The default is 300 seconds (5 minutes).
...
[gpload](gpload.md), `CREATE EXTERNAL TABLE`
