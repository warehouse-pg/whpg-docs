...
-   [Allowing Connections to WarehousePG Database](#topic_ln1_ptd_jr)
...
## <a id="topic_ln1_ptd_jr"></a>Allowing Connections to WarehousePG Database 
...
`host`
:   Matches connection attempts made using TCP/IP. Remote TCP/IP connections will not be possible unless the server is started with an appropriate value for the `listen_addresses` server configuration parameter. WarehousePG Database by default allows connections from all hosts \(`'*'`\).
...
> **Note** WarehousePG Command Center provides an interface for editing the `pg_hba.conf` file. It verifies entries before you save them, keeps a version history of the file so that you can reload a previous version of the file, and reloads the file into WarehousePG Database.
...
You can make changes to the OpenSSL configuration by updating the `openssl.cnf` file under your OpenSSL installation directory, or the file referenced by `$OPENSSL_CONF`, if present, and then restarting the WarehousePG Database server.
...
The default location for the following SSL server files is the WarehousePG Database coordinator data directory \(`$COORDINATOR_DATA_DIRECTORY`\):
...
The "PAM" \(Pluggable Authentication Modules\) authentication method validates username/password pairs, similar to basic authentication. To use PAM authentication, the user must already exist as a WarehousePG Database role name.
...
WarehousePG Database does not install a PAM configuration file. If you choose to use PAM authentication with WarehousePG, you must identify the PAM service name for WarehousePG and create the associated PAM service configuration file and configure WarehousePG Database to use PAM authentication as described below:
...
2.  Identify the `pamservice` name for WarehousePG Database. In this procedure, we choose the name `greenplum`.
...
5.  Add one or more entries to the `pg_hba.conf` configuration file to enable PAM authentication in WarehousePG Database. These entries must specify the `pam` *auth-method*. You must also specify the `pamservice=greenplum` *auth-option*. For example:
...
6.  Reload the WarehousePG Database configuration:
...
1.  Stop your WarehousePG Database system:
...
4.  Restart your WarehousePG Database system:
