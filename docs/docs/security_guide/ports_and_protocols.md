# WarehousePG Database Ports and Protocols 
...
> **Note** To avoid port conflicts between WarehousePG Database and other applications when initializing WarehousePG Database, do not specify WarehousePG Database ports in the range specified by the operating system parameter `net.ipv4.ip_local_port_range`. For example, if `net.ipv4.ip_local_port_range = 10000 65535`, you could set the WarehousePG Database base port numbers to values outside of that range:
...
Some add-on products and services that work with WarehousePG Database have additional networking requirements. The following table lists ports and protocols used within the WarehousePG cluster, and includes services and applications that integrate with WarehousePG Database.
...
|WarehousePG Database Interconnect|UDP 1025-65535, dynamically allocated|The Interconnect transports database tuples between WarehousePG segments during query execution.|
...
|WarehousePG Database file load and transfer utilities: gpfdist, gpload.|TCP 8080, HTTP<br/>TCP 9000, HTTPS|The gpfdist file serving utility can run on WarehousePG hosts or external hosts. Specify the connection port with the `-p` option when starting the server.<br/><br/>The gpload utility runs one or more instances of gpfdist with ports or port ranges specified in a configuration file.|
...
|WarehousePG Database secure shell \(SSH\): gpssh, gpsync, gpssh-exkeys, gppkg|TCP 22, SSH|Many WarehousePG utilities use rsync and ssh to transfer files between hosts and manage the WarehousePG system within the cluster.|
|WarehousePG Platform Extension Framework \(PXF\)|TCP 5888|The PXF Java service runs on port number 5888 on each WarehousePG Database segment host.|
|WarehousePG Command Center \(GPCC\)|TCP 28080, HTTP/HTTPS, WebSocket \(WS\), Secure WebSocket \(WSS\)|The GPCC web server \(`gpccws` process\) runs on the WarehousePG Database coordinator host or standby coordinator host. The port number is configured at installation time.|
|WarehousePG Command Center \(GPCC\)|TCP 8899, rcp port|A GPCC agent \(`ccagent` process\) on each WarehousePG Database segment host connects to the GPCC rpc backend at port number 8899 on the GPCC web server host.|
|WarehousePG Command Center \(GPCC\)|UNIX domain socket, agent|WarehousePG Database processes transmit datagrams to the GPCC agent \(`ccagent` process\) on each segment host using a UNIX domain socket.|
