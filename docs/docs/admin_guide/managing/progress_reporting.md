...
WarehousePG Database can report the progress of `ANALYZE`, `CLUSTER`, `COPY`, `CREATE INDEX`, `REINDEX`, and `VACUUM` commands during command execution. WarehousePG can also report the progress of a running base backup (initiated during [gprecoverseg -F](../../utility_guide/ref/gprecoverseg.html)) command invocation, allowing you to monitor the progress of these possibly long-running operations.
...
For each active analyze operation, the `gp_stat_progress_analyze_summary` view aggregates across the WarehousePG Database cluster the metrics reported by `gp_stat_progress_analyze`.
...
The [gp_stat_progress_cluster](../../ref_guide/system_catalogs/catalog_ref-views.html#gp_stat_progress_cluster) system view reports the progress of running `CLUSTER`, `clusterdb`, and `VACUUM FULL` (on a heap table) operations. (`VACUUM FULL` on a heap table is similar to `CLUSTER` in that WarehousePG performs a re-write of the table.) The view displays a row per segment instance that is currently servicing any of the mentioned commands.

For each active cluster or vacuum full operation, the `gp_stat_progress_cluster_summary` view aggregates across the WarehousePG Database cluster the metrics reported by `gp_stat_progress_cluster`.
...
For each active copy operation, the `gp_stat_progress_copy_summary` aggregates across the WarehousePG Database cluster the metrics reported by `gp_stat_progress_copy`.
...
For each active index operation, the `gp_stat_progress_create_index_summary` view aggregates across the WarehousePG Database cluster the metrics reported by `gp_stat_progress_create_index`.
...
| `waiting for writers before build` | This phase is not applicable, WarehousePG Database does not support `CREATE INDEX CONCURRENTLY` or `REINDEX CONCURRENTLY`. |
...
| `waiting for writers before validation` | This phase is not applicable, WarehousePG Database does not support `CREATE INDEX CONCURRENTLY` or `REINDEX CONCURRENTLY`. |
| `index validation: scanning index` | This phase is not applicable, WarehousePG Database does not support `CREATE INDEX CONCURRENTLY` or `REINDEX CONCURRENTLY`. |
| `index validation: sorting tuples` | This phase is not applicable, WarehousePG Database does not support `CREATE INDEX CONCURRENTLY` or `REINDEX CONCURRENTLY`. |
| `index validation: scanning table` | This phase is not applicable, WarehousePG Database does not support `CREATE INDEX CONCURRENTLY` or `REINDEX CONCURRENTLY`. |
| `waiting for old snapshots` | This phase is not applicable, WarehousePG Database does not support `CREATE INDEX CONCURRENTLY` or `REINDEX CONCURRENTLY`. |
| `waiting for readers before marking dead` | This phase is not applicable, WarehousePG Database does not support `REINDEX CONCURRENTLY`. |
| `waiting for readers before dropping` | This phase is not applicable, WarehousePG Database does not support `REINDEX CONCURRENTLY`. |

WarehousePG Database skips several phases because it does not support concurrent index creation or concurrent reindexing.
...
For each active vacuum operation, the `gp_stat_progress_vacuum_summary` view aggregates across the WarehousePG Database cluster the metrics reported by `gp_stat_progress_vacuum`.
...
For each active base backup operation, the `gp_stat_progress_basebackup_summary` view aggregates across the WarehousePG Database cluster the metrics reported by `gp_stat_progress_basebackup`.
...
Take note of these additional considerations related to progress reporting in WarehousePG Database:
