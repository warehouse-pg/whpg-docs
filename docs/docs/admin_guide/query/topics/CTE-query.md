...
> **Note** Limitations when using a `WITH` clause in WarehousePG Database include:
> 
...
By default, WarehousePG Database enables the `RECURSIVE` keyword for the `WITH` clause. `RECURSIVE` can be deactivated by setting the server configuration parameter [gp\_recursive\_cte](../../../ref_guide/config_params/guc-list.html) to `false`.
...
2.  As long as the working table is not empty, repeat these steps:
    ...
    > **Note** While `RECURSIVE` allows queries to be specified recursively, WarehousePG Database evaluates such queries iteratively internally.
...
In particular, if there's an index on `key`, WarehousePG Database uses it to fetch just the rows having `key = 123`. On the other hand, in:
...
Here, materialization of the `WITH` query ensures that WarehousePG Database evaluations `very_expensive_function` only once per table row, not twice.
...
- WarehousePG Database inlines multiply-referenced CTEs only when requested (by specifying `NOT MATERIALIZED`).
- Multiply-referenced CTEs cannot be inlined if they contain outer self-references.
- WarehousePG Database does not inline when the CTE includes a volatile function.
- An `ORDER BY` in the subquery or CTE does not force an ordering for the whole query.
- WarehousePG Database always materializes a CTE term in a query. Due to this:
