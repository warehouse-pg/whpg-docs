...
Use of this data type requires the installation to have been built with `configure --with-libxml`. This is enabled by default for VMware WarehousePG builds.
...
The above method converts character strings into XML values according to the SQL standard, but you can also use WarehousePG Database syntax like the following:
...
type can be `character`, `character varying`, or `text` \(or an alias for one of those\). Again, according to the SQL standard, this is the only way to convert between type `xml` and character types, but WarehousePG Database also allows you to simply cast the value.
...
or simply like WarehousePG Database:
...
To process values of data type `xml`, WarehousePG Database offers the functions `xpath` and `xpath_exists`, which evaluate XPath 1.0 expressions.
...
The `BY REF` clauses have no effect in WarehousePG Database, but are allowed for SQL conformance and compatibility with other implementations. Per SQL standard, the first `BY REF` is required, the second is optional. Also note that the SQL standard specifies the `xmlexists` construct to take an XQuery expression as first argument, but WarehousePG Database currently only supports XPath, which is a subset of XQuery.
