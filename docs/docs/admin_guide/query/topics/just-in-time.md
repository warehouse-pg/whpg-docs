...
This topic provides an explanation of what Just-in-Time compilation is and how to configure it in WarehousePG Database.
...
WarehousePG Database uses LLVM for JIT compilation and it is enabled with all RPM distributions of WarehousePG Database. If you build WarehousePG Database from source, you must include the `--with-llvm` build option to include JIT compilation support.
...
Currently WarehousePG Database's JIT implementation supports for accelerating expression evaluation and tuple deforming.
...
1. Planner Stage
    ...
    This stage takes place in the WarehousePG Database coordinator. The planner generates the plan tree of a query and its estimated cost. By default, WarehousePG Database uses GPORCA to generate a query plan. Otherwise it uses Postgres-based planner as a fallback method.
    ...
    Verify the values of these configuration parameters for both GPORCA and Postgres-based planner, as the meaning of cost is different. When using GPORCA, WarehousePG Database may sometimes fall back to Postgres-based planner for some operations. Note that setting the JIT cost parameters to ‘0’ forces all queries to be JIT-compiled and, as a result, slows down queries. Setting them to a negative value will disable the feature the parameter provides.
