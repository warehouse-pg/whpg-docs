...
For WarehousePG Database, the transaction ID \(XID\) value an incrementing 32-bit \(2<sup>32</sup>\) value. The maximum unsigned 32-bit value is 4,294,967,295, or about four billion. The XID values restart at 3 after the maximum is reached. WarehousePG Database handles the limit of XID values with two features:

-   Calculations on XID values using modulo-2<sup>32</sup> arithmetic that allow WarehousePG Database to reuse XID values. The modulo calculations determine the order of transactions, whether one transaction has occurred before or after another, based on the XID.
    ...
    Using the WarehousePG Database modulo calculations, as long as two XIDs are within 2<sup>31</sup> transactions of each other, comparing them yields the correct result.

-   A frozen XID value that WarehousePG Database uses as the XID for current \(visible\) data rows. Setting a row's XID to the frozen XID performs two functions.
    -   When WarehousePG Database compares XIDs using the modulo calculations, the frozen XID is always smaller, earlier, when compared to any other XID. If a row's XID is not set to the frozen XID and 2<sup>31</sup> new transactions are run, the row appears to be run in the future based on the modulo calculation.
...
**Parent topic:** [About Concurrency Control in WarehousePG Database](../intro/about_mvcc.html)
...
-   The frozen XID is -2. This is different than the WarehousePG Database frozen XID.
...
For this simple example, the database is close to running out of available XID values. When WarehousePG Database is close to running out of available XID values, WarehousePG Database takes these actions.

-   WarehousePG Database issues a warning stating that the database is running out of XID values.

    ```
    WARNING: database "<database_name>" must be vacuumed within <number_of_transactions> transactions
    ```

-   Before the last XID is assigned, WarehousePG Database stops accepting transactions to prevent assigning an XID value twice and issues this message.
...
To manage transaction IDs and table data that is stored on disk, WarehousePG Database provides the `VACUUM` command.
...
-   A `VACUUM` operation manages obsolete or deleted table rows on disk. This database's `VACUUM` command changes the XID values `obsolete` to indicate obsolete rows. A WarehousePG Database `VACUUM` operation, without the `FULL` option, deletes the data opportunistically to remove rows on disk with minimal impact to performance and data availability.

For the example table, a `VACUUM` operation has been performed on the table. The command updated table data on disk. This version of the `VACUUM` command performs slightly differently than the WarehousePG Database command, but the concepts are the same.
...
Obsolete rows can be deleted from disk. For WarehousePG Database, the `VACUUM` command, with `FULL` option, does more extensive processing to reclaim disk space.
...
When performing the modulo calculations that compare XIDs, WarehousePG Database, considers the XIDs of the rows and the current range of available XIDs to determine if XID wrapping has occurred between row XIDs.
