...
Before you begin installing WarehousePG, be sure you have completed the steps in [Configure Operating System](config_os.md) to configure each of the coordinator, standby coordinator, and segment host machines for WarehousePG.

> **Important** After installing WarehousePG, you must set WarehousePG environment variables. See [Setting WarehousePG Environment Variables](init_whpg.md).

See [Example Ansible Playbook](ansible-example.md) for an example script that shows how you can automate creating the `gpadmin` user and installing the WarehousePG.
...
-   `greenplum_path.sh` — This file contains the environment variables for WarehousePG. See [Setting WarehousePG Environment Variables](init_whpg.md).
