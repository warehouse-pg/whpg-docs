...
> **Important** Set the WarehousePG timezone to a value that is compatible with your host systems. Setting the WarehousePG timezone prevents WarehousePG from selecting a timezone each time the cluster is restarted and sets the timezone for the WarehousePG coordinator and segment instances. After you upgrade to this release and if you have not set a WarehousePG timezone value, verify that the selected WarehousePG timezone is acceptable for your deployment. See [Configuring Timezone and Localization Settings](localization.md) for more information.
...
12. If you configured PXF in your previous WarehousePG installation, you may need to install PXF in your new WarehousePG installation, and you may be required to re-initialize or register the PXF service after you upgrade WarehousePG. Refer to the [Step 2](../pxf/upgrade_pxf_6x.md#pxfup) PXF upgrade procedure for instructions.
