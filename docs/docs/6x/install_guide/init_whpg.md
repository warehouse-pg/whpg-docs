...
The instructions in this chapter assume you have already prepared your hosts as described in [Configure Operating System](config_os.md) and installed the WarehousePG software on all of the hosts in the system according to the instructions in [Installing the WarehousePG Software](install_whpg.md).
...
1.  Make sure you have completed all of the installation tasks described in [Configure Operating System](config_os.md) and [Installing the WarehousePG Software](install_whpg.md).
...
4.  By default, WarehousePG will be initialized using the locale of the coordinator host system. Make sure this is the correct locale you want to use, as some locale options cannot be changed after initialization. See [Configuring Timezone and Localization Settings](localization.md) for more information.
...
Your WarehousePG configuration file tells the [gpinitsystem](../ref_guide/utility_guide/reference/gpinitsystem.md) utility how you want to configure your WarehousePG cluster. An example configuration file can be found in `$GPHOME/docs/cli_help/gpconfigs/gpinitsystem_config`.
...
2.  Open the file you just copied in a text editor.
    ...
    To specify `PORT_BASE`, review the port range specified in the `net.ipv4.ip_local_port_range` parameter in the `/etc/sysctl.conf` file. See [Recommended OS Parameters Settings](config_os.md).
...
3.  (Optional) If you want to deploy mirror segments, uncomment and set the mirroring parameters according to your environment. To specify `MIRROR_PORT_BASE`, review the port range specified under the `net.ipv4.ip_local_port_range` parameter in the `/etc/sysctl.conf` file. Here is an example of the *optional* mirror parameters in the `gpinitsystem_config` file:
    ...
    > **Note** You can initialize your WarehousePG cluster with primary segments only and deploy mirrors later using the [gpaddmirrors](../ref_guide/utility_guide/reference/gpaddmirrors.md) utility.
...
The [gpinitsystem](../ref_guide/utility_guide/reference/gpinitsystem.md) utility will create a WarehousePG cluster using the values defined in the configuration file.
...
1.  Run the following command referencing the path and file name of your initialization configuration file (`gpinitsystem_config`) and host file (`hostfile_gpinitsystem`). For example:
    ...
    This output file can be edited and used at a later stage as the input file of the `-I` option, to create a new cluster or to recover from a backup. See [gpinitsystem](../ref_guide/utility_guide/reference/gpinitsystem.md) for further details.
...
Use the [gpconfig](../ref_guide/utility_guide/reference/gpconfig.md) utility to show and set the WarehousePG timezone. For example, these commands show the WarehousePG timezone and set the timezone to `US/Pacific`.
...
For more information about the WarehousePG timezone, see [Configuring Timezone and Localization Settings](localization.md).
...
After verifying your installation, you may want to begin creating databases and loading data. See [DDL: Defining Database Objects](../admin_guide/ddl/index.md) and [Loading and Unloading Data](../admin_guide/load/index.md) in the *WarehousePG Administrator Guide* for more information about creating databases, schemas, tables, and other database objects in WarehousePG and loading your data.
