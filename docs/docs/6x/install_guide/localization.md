...
Locale support is automatically initialized when a WarehousePG cluster is initialized. The initialization utility, [gpinitsystem](../ref_guide/utility_guide/reference/gpinitsystem.md), will initialize the WarehousePG cluster with the locale setting of its execution environment by default, so if your system is already set to use the locale that you want in your WarehousePG cluster then there is nothing else you need to do.
...
See [Initializing WarehousePG](init_whpg.md) for information about the database initialization process.
...
Check that WarehousePG is actually using the locale that you think it is. `LC_COLLATE` and `LC_CTYPE` settings are determined at initialization time and cannot be changed without redoing [gpinitsystem](../ref_guide/utility_guide/reference/gpinitsystem.md). Other locale settings including `LC_MESSAGES` and `LC_MONETARY` are initially determined by the operating system environment of the coordinator and/or segment host, but can be changed after initialization by editing the `postgresql.conf` file of each WarehousePG coordinator and segment instance. You can check the active locale settings of the coordinator host using the `SHOW` command. Note that every host in your WarehousePG array should be using identical locale settings.
