...
For predefined aggregate functions, see [Built-in Functions and Operators](functions-operators.md). You can also add custom aggregate functions.
...
Where *`window_function`* is one of the functions listed in [Window functions](functions-operators.md#window-functions) or a user-defined window function, *`expression`* is any value expression that does not contain a window expression, and *`window_specification`* is:
