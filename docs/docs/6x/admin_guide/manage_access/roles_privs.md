...
In its default configuration, WarehousePG saves MD5 hashes of login users' passwords in the [pg_authid](../../ref_guide/system_catalogs/system_catalogs_definitions/pg_authid.md) system catalog rather than saving clear text passwords. Anyone who is able to view the `pg_authid` table can see hash strings, but no passwords. This also ensures that passwords are obscured when the database is dumped to backup files.

WarehousePG supports SHA-256 and SCRAM-SHA-256 password hash algorithms as well. The [password_hash_algorithm](../../ref_guide/config_params/guc-list.md#password_hash_algorithm) server configuration parameter value and pg_hba.conf settings determine how passwords are hashed and what authentication method is in effect.
