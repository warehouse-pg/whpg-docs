---
title: System Catalogs Definitions
navigation:
  - catalog_ref
  - catalog_ref-tables
  - catalog_ref-views
  - catalog_ref-html
  - foreign_data_wrapper_options
  - foreign_data_wrappers
  - foreign_server_options
  - foreign_servers
  - foreign_table_options
  - foreign_tables
  - gp_configuration_history
  - gp_distributed_log
  - gp_distributed_xacts
  - gp_distribution_policy
  - gpexpand_expansion_progress
  - gp_endpoints
  - gp_expansion_status
  - gp_expansion_tables
  - gp_fastsequence
  - gp_id
  - gp_pgdatabase
  - gp_resgroup_config
  - gp_resgroup_status
  - gp_resgroup_status_per_host
  - gp_resgroup_status_per_segment
  - gp_resqueue_status
  - gp_segment_configuration
  - gp_segment_endpoints
  - gp_session_endpoints
  - gp_stat_replication
  - gp_transaction_log
  - gp_version_at_initdb
  - pg_aggregate
  - pg_am
  - pg_amop
  - pg_amproc
  - pg_appendonly
  - pg_attrdef
  - pg_attribute
  - pg_attribute_encoding
  - pg_auth_members
  - pg_authid
  - pg_available_extension_versions
  - pg_available_extensions
  - pg_cast
  - pg_class
  - pg_compression
  - pg_constraint
  - pg_conversion
  - pg_cursors
  - pg_database
  - pg_db_role_setting
  - pg_depend
  - pg_description
  - pg_enum
  - pg_extension
  - pg_exttable
  - pg_foreign_data_wrapper
  - pg_foreign_server
  - pg_foreign_table
  - pg_index
  - pg_inherits
  - pg_language
  - pg_largeobject
  - pg_locks
  - pg_matviews
  - pg_max_external_files
  - pg_namespace
  - pg_opclass
  - pg_operator
  - pg_opfamily
  - pg_partition
  - pg_partition_columns
  - pg_partition_encoding
  - pg_partition_rule
  - pg_partition_templates
  - pg_partitions
  - pg_pltemplate
  - pg_proc
  - pg_resgroup
  - pg_resgroupcapability
  - pg_resourcetype
  - pg_resqueue
  - pg_resqueue_attributes
  - pg_resqueuecapability
  - pg_rewrite
  - pg_roles
  - pg_rules
  - pg_shdepend
  - pg_shdescription
  - pg_stat_activity
  - pg_stat_indexes
  - pg_stat_tables
  - pg_stat_last_operation
  - pg_stat_last_shoperation
  - pg_stat_operations
  - pg_stat_partition_operations
  - pg_stat_replication
  - pg_statistic
  - pg_stats_resqueue
  - pg_tablespace
  - pg_trigger
  - pg_type
  - pg_type_encoding
  - pg_user_mapping
  - pg_user_mappings
  - user_mapping_options
  - user_mappings
redirects:
  - catalog_ref

---

<a id="topic1"></a>

System catalog table and view definitions in alphabetical order.

-   [foreign_data_wrapper_options](foreign_data_wrapper_options.md)

-   [foreign_data_wrappers](foreign_data_wrappers.md)

-   [foreign_server_options](foreign_server_options.md)

-   [foreign_servers](foreign_servers.md)

-   [foreign_table_options](foreign_table_options.md)

-   [foreign_tables](foreign_tables.md)

-   [gp_configuration_history](gp_configuration_history.md)

-   [gp_distributed_log](gp_distributed_log.md)

-   [gp_distributed_xacts](gp_distributed_xacts.md)

-   [gp_distribution_policy](gp_distribution_policy.md)

-   [gpexpand.expansion_progress](gpexpand_expansion_progress.md)

-   [gp_endpoints](gp_endpoints.md)

-   [gpexpand.status](gp_expansion_status.md)

-   [gpexpand.status_detail](gp_expansion_tables.md)

-   [gp_fastsequence](gp_fastsequence.md)

-   [gp_id](gp_id.md)

-   [gp_pgdatabase](gp_pgdatabase.md)

-   [gp_resgroup_config](gp_resgroup_config.md)

-   [gp_resgroup_status](gp_resgroup_status.md)

-   [gp_resgroup_status_per_host](gp_resgroup_status_per_host.md)

-   [gp_resgroup_status_per_segment](gp_resgroup_status_per_segment.md)

-   [gp_resqueue_status](gp_resqueue_status.md)

-   [gp_segment_configuration](gp_segment_configuration.md)

-   [gp_segment_endpoints](gp_segment_endpoints.md)

-   [gp_session_endpoints](gp_session_endpoints.md)

-   [gp_stat_archiver](gp_stat_archiver.md)

-   [gp_stat_replication](gp_stat_replication.md)

-   [gp_suboverflowed_backend](gp_suboverflowed_backend.md)

-   [gp_transaction_log](gp_transaction_log.md)

-   [gp_version_at_initdb](gp_version_at_initdb.md)

-   [pg_aggregate](pg_aggregate.md)

-   [pg_am](pg_am.md)

-   [pg_amop](pg_amop.md)

-   [pg_amproc](pg_amproc.md)

-   [pg_appendonly](pg_appendonly.md)

-   [pg_attrdef](pg_attrdef.md)

-   [pg_attribute](pg_attribute.md)

-   [pg_attribute_encoding](pg_attribute_encoding.md)

-   [pg_auth_members](pg_auth_members.md)

-   [pg_authid](pg_authid.md)

-   [pg_available_extension_versions](pg_available_extension_versions.md)

-   [pg_available_extensions](pg_available_extensions.md)

-   [pg_cast](pg_cast.md)

-   [pg_class](pg_class.md)

-   [pg_compression](pg_compression.md)

-   [pg_constraint](pg_constraint.md)

-   [pg_conversion](pg_conversion.md)

-   [pg_cursors](pg_cursors.md)

-   [pg_database](pg_database.md)

-   [pg_db_role_setting](pg_db_role_setting.md)

-   [pg_depend](pg_depend.md)

-   [pg_description](pg_description.md)

-   [pg_enum](pg_enum.md)

-   [pg_extension](pg_extension.md)

-   [pg_exttable](pg_exttable.md)

-   [pg_foreign_data_wrapper](pg_foreign_data_wrapper.md)

-   [pg_foreign_server](pg_foreign_server.md)

-   [pg_foreign_table](pg_foreign_table.md)

-   [pg_index](index.md)

-   [pg_inherits](pg_inherits.md)

-   [pg_language](pg_language.md)

-   [pg_largeobject](pg_largeobject.md)

-   [pg_locks](pg_locks.md)

-   [pg_matviews](pg_matviews.md)

-   [pg_max_external_files](pg_max_external_files.md)

-   [pg_namespace](pg_namespace.md)

-   [pg_opclass](pg_opclass.md)

-   [pg_operator](pg_operator.md)

-   [pg_opfamily](pg_opfamily.md)

-   [pg_partition](pg_partition.md)

-   [pg_partition_columns](pg_partition_columns.md)

-   [pg_partition_encoding](pg_partition_encoding.md)

-   [pg_partition_rule](pg_partition_rule.md)

-   [pg_partition_templates](pg_partition_templates.md)

-   [pg_partitions](pg_partitions.md)

-   [pg_pltemplate](pg_pltemplate.md)

-   [pg_proc](pg_proc.md)

-   [pg_resgroup](pg_resgroup.md)

-   [pg_resgroupcapability](pg_resgroupcapability.md)

-   [pg_resourcetype](pg_resourcetype.md)

-   [pg_resqueue](pg_resqueue.md)

-   [pg_resqueue_attributes](pg_resqueue_attributes.md)

-   [pg_resqueuecapability](pg_resqueuecapability.md)

-   [pg_rewrite](pg_rewrite.md)

-   [pg_roles](pg_roles.md)

-   [pg_rules](pg_rules.md)

-   [pg_shdepend](pg_shdepend.md)

-   [pg_shdescription](pg_shdescription.md)

-   [pg_stat_activity](pg_stat_activity.md)

-   [pg_stat_all_indexes](pg_stat_indexes.md)

-   [pg_stat_all_tables](pg_stat_tables.md)

-   [pg_stat_last_operation](pg_stat_last_operation.md)

-   [pg_stat_last_shoperation](pg_stat_last_shoperation.md)

-   [pg_stat_operations](pg_stat_operations.md)

-   [pg_stat_partition_operations](pg_stat_partition_operations.md)

-   [pg_stat_replication](pg_stat_replication.md)

-   [pg_statistic](pg_statistic.md)

-   [pg_stat_resqueues](pg_stats_resqueue.md)

-   [pg_tablespace](pg_tablespace.md)

-   [pg_trigger](pg_trigger.md)

-   [pg_type](pg_type.md)

-   [pg_type_encoding](pg_type_encoding.md)

-   [pg_user_mapping](pg_user_mapping.md)

-   [pg_user_mappings](pg_user_mappings.md)

-   [user_mapping_options](user_mapping_options.md)

-   [user_mappings](user_mappings.md)

Parent topic: [WarehousePG Reference Guide](../index.md)
