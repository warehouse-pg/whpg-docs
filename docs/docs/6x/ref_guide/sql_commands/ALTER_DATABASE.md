...
-   parameter value
    ...
    Set this database's session default for the specified configuration parameter to the given value. If value is `DEFAULT` or, equivalently, `RESET` is used, the database-specific setting is removed, so the system-wide default setting will be inherited in new sessions. Use `RESET ALL` to clear all database-specific settings. See [Server Configuration Parameters](../config_params/index.md) for information about all user-settable configuration parameters.
...
[CREATE DATABASE](CREATE_DATABASE.md), [DROP DATABASE](DROP_DATABASE.md), [SET](SET.md), [CREATE TABLESPACE](CREATE_TABLESPACE.md)
