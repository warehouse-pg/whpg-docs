...
In addition, you can define your own range types; see [CREATE TYPE](../sql_commands/CREATE_TYPE.md) for more information.
...
See [Range Functions and Operators](../function-summary.md) for complete lists of operators and functions on range types.
...
In the text form of a range, an inclusive lower bound is represented by `[` while an exclusive lower bound is represented by `(` . Likewise, an inclusive upper bound is represented by `]` , while an exclusive upper bound is represented by `)` . (See [Range Functions and Operators](../function-summary.md) for more details.)
...
See [CREATE TYPE](../sql_commands/CREATE_TYPE.md) for more information about creating range types.
...
A GiST or SP-GiST index can accelerate queries involving these range operators: `=`, `&&`, `<@`, `@>`, `<<`, `>>`, `-|-`, `&<`, and `&>` (see [Range Functions and Operators](../function-summary.md) for more information).
