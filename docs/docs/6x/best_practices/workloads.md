...
> **Note** Resource groups are a newer resource management scheme that enforces memory, CPU, and concurrent transaction limits in WarehousePG. The [Managing Resources](../admin_guide/performance/wlmgmt/index.md) topic provides a comparison of the resource queue and the resource group management schemes. Refer to [Using Resource Groups](../admin_guide/performance/wlmgmt/workload_mgmt_resgroups.md) for configuration and usage information for this resource management scheme.
...
-   **vm.overcommit_memory**
    ...
    This is a Linux kernel parameter, set in `/etc/sysctl.conf` and it should always be set to 2. It determines the method the OS uses for determining how much memory can be allocated to processes and 2 is the only safe setting for WarehousePG. Please review the sysctl parameters in the [WarehousePG Installation Guide](../install_guide/config_os.md#sysctl_file).

-   **vm.overcommit_ratio**

    This is a Linux kernel parameter, set in [`/etc/sysctl.conf`](../install_guide/config_os.md#sysctl_file). It is the percentage of RAM that is used for application processes. The remainder is reserved for the operating system. The default on Red Hat is 50.
    ...
    See [WarehousePG Memory Overview](../admin_guide/performance/wlmgmt_intro.md) for instructions to calculate a value for `vm.overcommit_ratio`.
