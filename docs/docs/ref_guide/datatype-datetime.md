...
WarehousePG Database 6 and later releases do not automatically cast text from the deprecated timestamp format `YYYYMMDDHH24MISS`. The format could not be parsed unambiguously in previous WarehousePG Database releases.

For example, this command returns an error in WarehousePG Database 6. In previous releases, a timestamp is returned.

```
# select to_timestamp('20190905140000');
```

In WarehousePG Database 6, this command returns a timestamp.
