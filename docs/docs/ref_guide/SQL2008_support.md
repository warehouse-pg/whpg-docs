...
The following table lists the features described in the 2008 SQL standard. Features that are supported in WarehousePG Database are marked as YES in the 'Supported' column, features that are not implemented are marked as NO.

For information about WarehousePG features and SQL compliance, see the *WarehousePG Database Administrator Guide*.
...
|E061-08|`EXISTS` predicate|YES|Not all uses work in WarehousePG|
...
|E141-02|`UNIQUE` constraints of `NOT NULL` columns|YES|Must be the same as or a superset of the WarehousePG distribution key|
|E141-03|`PRIMARY KEY` constraints|YES|Must be the same as or a superset of the WarehousePG distribution key|
...
|E141-10|Names in a foreign key can be specified in any order|YES|Foreign keys can be declared but are not enforced in WarehousePG|
...
|S091|Basic array support|NO|WarehousePG has arrays, but is not fully standards compliant|
...
|S201|SQL-invoked routines on arrays|NO|Functions can be passed WarehousePG array types|
...
|T618|`NTH_VALUE`|NO|Function exists in WarehousePG but not all options are supported|
