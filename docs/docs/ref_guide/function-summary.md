...
WarehousePG Database supports built-in functions and operators including analytic functions and window functions that can be used in window expressions. For information about using built-in WarehousePG Database functions see, "Using Functions and Operators" in the *WarehousePG Database Administrator Guide*.

-   [WarehousePG Database Function Types](#topic27)
...
## <a id="topic27"></a>WarehousePG Database Function Types 

WarehousePG Database evaluates functions and operators used in SQL expressions. Some functions and operators are only allowed to run on the coordinator since they could lead to inconsistencies in WarehousePG Database segment instances. This table describes the WarehousePG Database Function Types.
...
WarehousePG Database does not support functions that return a table reference \(`rangeFuncs`\) or functions that use the `refCursor` datatype.

## <a id="topic29"></a>Built-in Functions and Operators 

The following table lists the categories of built-in functions and operators supported by PostgreSQL. All functions and operators are supported in WarehousePG Database as in PostgreSQL with the exception of `STABLE` and `VOLATILE` functions, which are subject to the restrictions noted in [WarehousePG Database Function Types](#topic27). See the [Functions and Operators](https://www.postgresql.org/docs/12/functions.html) section of the PostgreSQL documentation for more information about these built-in functions and operators.
...
WarehousePG Database includes built-in functions and operators that create and manipulate JSON data:
...
WarehousePG Database's implementation of SQL/JSON path language has the following deviations from the SQL/JSON standard:

- `.datetime()` item method is not implemented yet mainly because immutable `jsonpath` functions and operators cannot reference session timezone, which is used in some datetime operations. Datetime support will be added to `jsonpath` in future versions of WarehousePG Database.
- A path expression can be a Boolean predicate, although the SQL/JSON standard allows predicates only in filters. This is necessary for implementation of the `@@` operator. For example, the following `jsonpath` expression is valid in WarehousePG Database:
...
The following are WarehousePG Database built-in window functions. All window functions are *immutable*. For more information about window functions, see "Window Expressions" in the *WarehousePG Database Administrator Guide*.
...
> **Note** The WarehousePG MADlib Extension for Analytics provides additional advanced functions to perform statistical analysis and machine learning with WarehousePG Database data. See [MADlib Extension for Analytics](../analytics/madlib.html#topic1).
...
The following tables summarize the functions and operators that are provided for full text searching. See [Using Full Text Search](../admin_guide/textsearch/full-text-search.html) for a detailed explanation of WarehousePG Database's text search facility.
