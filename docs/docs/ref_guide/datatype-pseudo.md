...
WarehousePG Database supports special-purpose data type entries that are collectively called *pseudo-types*. A pseudo-type cannot be used as a column data type, but it can be used to declare a function's argument or result type. Each of the available pseudo-types is useful in situations where a function's behavior does not correspond to simply taking or returning a value of a specific SQL data type.
...
> **Note** WarehousePG Database does not support triggers and the pseudo-type *trigger*.
...
The pseudo-type *anytable* is a WarehousePG Database type that specifies a table expression—an expression that computes a table. WarehousePG Database allows this type only as an argument to a user-defined function. See [Table Value Expressions](#topic_ig2_1pc_qfb) for more about the *anytable* pseudo-type.
...
The *anytable* type is only permitted in functions implemented in the C or C++ languages. The body of the function can access the table using the WarehousePG Database Server Programming Interface \(SPI\) or the WarehousePG Partner Connector \(GPPC\) API.

The *anytable* type is used in some user-defined functions in the VMware WarehousePG Text API. The following GPText example uses the `TABLE` function with the `SCATTER BY` clause in the GPText function `gptext.index()` to populate the index `mydb.mytest.articles` with data from the messages table:
...
For information about the function `gptext.index()`, see the VMware WarehousePG Text documentation.
