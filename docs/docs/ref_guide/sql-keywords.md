...
This topic describes WarehousePG Database reserved identifiers and object names, and SQL key words recognized by the WarehousePG Database and PostgreSQL command parsers.

## <a id="resident"></a>Reserved Identifiers 

In the WarehousePG Database system, names beginning with `gp_` and `pg_` are reserved and should not be used as names for user-created objects, such as tables, views, and functions.
...
[Table 1](#table_yjz_3hb_jhb) lists all tokens that are key words in WarehousePG Database 6 and PostgreSQL 9.4.
...
|Key Word|WarehousePG Database|PostgreSQL 9.4|
|:-------|:-----------------|:-------------|
