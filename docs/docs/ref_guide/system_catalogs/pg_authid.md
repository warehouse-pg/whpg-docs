...
Because user identities are system-wide, `pg_authid` is shared across all databases in a WarehousePG Database system: there is only one copy of `pg_authid` per system, not one per database.
