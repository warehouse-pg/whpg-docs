...
The `foreign_table_options` view contains all of the options defined for foreign tables in the current database. WarehousePG Database displays only those foreign tables to which the current user has access \(by way of being the owner or having some privilege\).
