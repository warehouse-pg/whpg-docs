...
-   [WarehousePG Database Array Configuration Parameters](#topic49)
...
These parameters control how clients connect and authenticate to WarehousePG Database.
...
These parameters set the limits for system resources consumed by WarehousePG Database.
...
> **Caution** Do not use cost-based vacuum delay because it runs asynchronously among the segment instances. The vacuum cost limit and delay is invoked at the segment level without taking into account the state of the entire WarehousePG Database array
...
These parameters control the usage of GPORCA by WarehousePG Database. For information about GPORCA, see [About GPORCA](../../admin_guide/query/topics/query-piv-optimizer.html) in the *WarehousePG Database Administrator Guide*.
...
> **Caution** Do not adjust these query costing parameters. They are tuned to reflect WarehousePG Database hardware configurations and typical workloads. All of these parameters are related. Changing one without changing the others can have adverse affects on performance.
...
These configuration parameters control WarehousePG Database logging.
...
These parameters enable and configure query metrics collection. When enabled, WarehousePG Database saves metrics to shared memory during query execution. These metrics are used by VMware WarehousePG Command Center, which is included with VMware's commercial version of WarehousePG Database.
...
The following configuration parameters configure the WarehousePG Database resource management feature \(resource queues\), query prioritization, memory utilization and concurrency control.
...
The following parameters configure the WarehousePG Database resource group workload management feature.
...
The following parameters configure the external tables feature of WarehousePG Database.
...
The following parameter configures default option settings for WarehousePG Database tables.
...
The following parameters configure the append-optimized tables feature of WarehousePG Database.
...
The following parameters provide compatibility with older PostgreSQL and WarehousePG Database versions. You do not need to change these parameters in WarehousePG Database.
...
### <a id="topic_jq1_n1v_bdb"></a>WarehousePG Database 

- [gp_ignore_error_table](guc-list.html#gp_ignore_error_table)

## <a id="topic49"></a>WarehousePG Database Array Configuration Parameters 

The parameters in this topic control the configuration of the WarehousePG Database array and its components: segments, coordinator, distributed transaction manager, coordinator mirror, and interconnect.
...
> **Note** WarehousePG Database supports only the UDPIFC \(default\) and TCP interconnect types.
...
These parameters control the configuration of the replication between WarehousePG Database primary coordinator and standby coordinator.
...
The parameters in this topic control the configuration of the WarehousePG Database PL/Java language.
...
The parameters in this topic control the configuration of the WarehousePG Database XML data type.
