...
Descriptions of the WarehousePG Database server configuration parameters listed alphabetically.
...
This controls whether the array input parser recognizes unquoted NULL as specifying a null array element. By default, this is on, allowing array values containing null values to be entered. WarehousePG Database versions before 3.0 did not support null values in arrays, and therefore would treat NULL as specifying a normal array element with the string value 'NULL'.
...
When enabled, WarehousePG Database starts up the autovacuum daemon, which operates at the database level. When the daemon is running, WarehousePG Database:
...
Specifies the number of pages after which WarehousePG Database flushes previously performed writes to disk. Whenever more than the specified amount of data has been written while performing a checkpoint, WarehousePG Database attempts to force the operating system to issue these writes to the underlying storage. Doing so limits the amount of dirty data in the kernel's page cache, reducing the likelihood of stalls when an `fsync` is issued at the end of the checkpoint, or when the OS writes data back in larger batches in the background. Often that results in greatly reduced transaction latency, but note that there are some cases where performance may degrade, especially with workloads that are bigger than [shared\_buffers](#shared_buffers), but smaller than the OS's page cache.
...
WarehousePG Database uses checksums to prevent loading data corrupted in the file system into memory managed by database processes. When heap data checksums are enabled, WarehousePG Database computes and stores checksums on heap data pages when they are written to disk. When a page is retrieved from disk, the checksum is verified. If the verification fails, an error is generated and the page is not permitted to load into managed memory.
...
> **Note** Setting `default_transaction_deferrable` to `on` has no effect in WarehousePG Database. Only read-only, `SERIALIZABLE` transactions can be deferred. However, WarehousePG Database does not support the `SERIALIZABLE` transaction isolation level. See [SET TRANSACTION](../sql_commands/SET_TRANSACTION.html).
...
Controls the default isolation level of each new transaction. WarehousePG Database treats `read uncommitted` the same as `read committed`, and treats `serializable` the same as `repeatable read`.
...
Sets the assumption about the effective size of the disk cache that is available to a single query for the Postgres-based planner. This is factored into estimates of the cost of using an index; a higher value makes it more likely index scans will be used, a lower value makes it more likely sequential scans will be used. When setting this parameter, you should consider both WarehousePG Database's shared buffers and the portion of the kernel's disk cache that will be used for data files \(though some data might exist in both places\). Take also into account the expected number of concurrent queries on different tables, since they will have to share the available space. This parameter has no effect on the size of shared memory allocated by a WarehousePG server instance, nor does it reserve kernel disk cache; it is used only for estimation purposes.
...
The `gp_autostats_allow_nonowner` server configuration parameter determines whether or not to allow WarehousePG Database to trigger automatic statistics collection when a table is modified by a non-owner.

The default value is `false`; WarehousePG Database does not trigger automatic statistics collection on a table that is updated by a non-owner.

When set to `true`, WarehousePG Database will also trigger automatic statistics collection on a table when:
...
When `autovacuum=on` (the default), specifies the types of tables that are eligible for automatic vacuuming of dead tuples. WarehousePG Database supports two `gp_autovacuum_scope` values:

- `catalog` - WarehousePG Database autovacuums catalog tables only (`pg_catalog` relations).
- `catalog_ao_aux` - WarehousePG Database autovacuums catalog tables and append-optimized auxiliary tables (`pg_catalog`, `pg_toast`, and `pg_aoseg` relations).
...
When a user starts a session with WarehousePG Database and issues a query, the system creates groups or 'gangs' of worker processes on each segment to do the work. After the work is done, the segment worker processes are destroyed except for a cached number which is set by this parameter. A lower setting conserves system resources on the segment hosts, but a higher setting may improve performance for power-users that want to issue many complex queries in a row.
...
Timeout for sending data to unresponsive WarehousePG Database user clients during query processing. A value of 0 deactivates the timeout, WarehousePG Database waits indefinitely for a client. When the timeout is reached, the query is cancelled with this message:
...
Controls table creation when a WarehousePG Database table is created with a CREATE TABLE or CREATE TABLE AS command that does not contain a DISTRIBUTED BY clause.

For CREATE TABLE, if the value of the parameter is `off` \(the default\), and the table creation command does not contain a DISTRIBUTED BY clause, WarehousePG Database chooses the table distribution key based on the command:
...
If the value of the parameter is set to `on`, WarehousePG Database follows these rules to create a table when the DISTRIBUTED BY clause is not specified:
...
For information about the Postgres-based planner and GPORCA, see "SQL: Querying Data" in the *WarehousePG Database Administrator Guide*.
...
-   appendoptimized
    ...
    > **Note** You use the `appendoptimized=value` syntax to specify the append-optimized table storage type. `appendoptimized` is a thin alias for the `appendonly` legacy storage option. WarehousePG Database stores `appendonly` in the catalog, and displays the same when listing the storage options for append-optimized tables.
...
4.  The value of `gp_default_storage_options` that is set for the WarehousePG Database system with the `gpconfig` utility
...
This example `gpconfig` utility command sets the default storage option for a WarehousePG Database system. If you set the defaults for multiple table storage options, the value must be enclosed in single quotes.

```
gpconfig -c 'gp_default_storage_options' -v 'appendoptimized=true, orientation=column'
```

This example `gpconfig` utility command shows the value of the parameter. The parameter value must be consistent across the WarehousePG Database coordinator and all segments.
...
Controls whether the WarehousePG Database Global Deadlock Detector is enabled to manage concurrent `UPDATE` and `DELETE` operations on heap tables to improve performance. See [DML: Inserting, Updating and Deleting](../../admin_guide/dml.html#topic_gdd) in the *WarehousePG Database Administrator Guide*. The default is `off`, the Global Deadlock Detector is deactivated.

If the Global Deadlock Detector is deactivated \(the default\), WarehousePG Database runs concurrent update and delete operations on a heap table serially.
...
Enables collection of query metrics. When query metrics collection is enabled, WarehousePG Database collects metrics during query execution. The default is off.

After changing this configuration parameter, WarehousePG Database must be restarted for the change to take effect.

The WarehousePG Database metrics collection extension, when enabled, sends the collected metrics over UDP to a VMware WarehousePG Command Center agent<sup>1</sup>.

> **Note** <sup>1</sup>The metrics collection extension is included in VMware's commercial version of WarehousePG Database. VMware WarehousePG Command Center is supported only with VMware WarehousePG.
...
## <a id="gp_external_enable_exec"></a>gp\_external\_enable\_exec 
 ...
 Activates or deactivates  the use of external tables that run OS commands or scripts on the segment hosts \(`CREATE EXTERNAL TABLE EXECUTE` syntax\). Must be enabled if using the VMware WarehousePG Command Center.
...
Enable filter pushdown when reading data from external tables. If pushdown fails, a query is run without pushing filters to the external data source \(instead, WarehousePG Database applies the same constraints to the result\). See [Defining External Tables](../../admin_guide/external/external-tables.html#topic3) for more information.
...
Specifies the maximum number of times that WarehousePG Database attempts to establish a primary-mirror replication connection. When this count is exceeded, the fault detection process \(`ftsprobe`\) stops retrying and marks the mirror down.
...
Controls the amount of parallel retrieve cursor endpoint detail that WarehousePG Database writes to the server log file.

The default value is `false`, WarehousePG Database does not log endpoint details to the log file. When set to `true`, WarehousePG writes endpoint detail information to the log file.
...
Controls the amount of information that is written to the log file about communication between WarehousePG Database segment instance worker processes. The default value is `terse`. The log information is written to both the coordinator and segment instance logs.
...
Controls WarehousePG Database behavior when the deprecated `INTO ERROR TABLE` clause is specified in a `CREATE EXTERNAL TABLE` or `COPY` command.

> **Note** The `INTO ERROR TABLE` clause was deprecated and removed in WarehousePG Database 5. In WarehousePG Database 7, this parameter will be removed as well, causing all `INTO ERROR TABLE` invocations be yield a syntax error.

The default value is `false`, WarehousePG Database returns an error if the `INTO ERROR TABLE` clause is specified in a command.

If the value is `true`, WarehousePG Database ignores the clause, issues a warning, and runs the command without the `INTO ERROR TABLE` clause. In WarehousePG Database 5.x and later, you access the error log information with built-in SQL functions. See the [CREATE EXTERNAL TABLE](../sql_commands/CREATE_EXTERNAL_TABLE.html) or [COPY](../sql_commands/COPY.html) command.

You can set this value to `true` to avoid the WarehousePG Database error when you run applications that run `CREATE EXTERNAL TABLE` or `COPY` commands that include the WarehousePG Database 4.3.x `INTO ERROR TABLE` clause.
...
For the parameter value *n*, WarehousePG Database stops processing input rows when you import data with the `COPY` command or from an external table if the first *n* rows processed contain formatting errors. If a valid row is processed within the first *n* rows, WarehousePG Database continues processing input rows.
...
The amount of shared memory, in kilobytes, allocated for query metrics. The default is 5120 and the maximum is 131072. At startup, if [gp\_enable\_query\_metrics](#gp_enable_query_metrics) is set to on, WarehousePG Database allocates space in shared memory to save query metrics. This memory is organized as a header and a list of slots. The number of slots needed depends on the number of concurrent queries and the number of execution plan nodes per query. The default value, 5120, is based on a WarehousePG Database system that runs a maximum of about 250 concurrent queries with 120 nodes per query. If the `gp_enable_query_metrics` configuration parameter is off, or if the slots are exhausted, the metrics are maintained in local memory instead of in shared memory.
...
Specifies the type of address binding strategy WarehousePG Database uses for communication between segment host sockets. There are two types: `unicast` and `wildcard`. The default is `unicast`.

- When this parameter is set to `unicast`, WarehousePG Database uses the `gp_segment_configuration.address` field to perform address binding. This reduces port usage on segment hosts and prevents interconnect traffic from being routed through unintended (and possibly slower) network interfaces. This is the recommended option.

- When this parameter is set to `wildcard`, WarehousePG Database uses a wildcard address for binding, enabling the use of any network interface compliant with routing rules.
...
Specifies the interval, in seconds, to log WarehousePG Database interconnect debugging messages when the server configuration parameter [gp\_log\_interconnect](#gp_log_interconnect) is set to `DEBUG`. The default is 10 seconds.

The log messages contain information about the interconnect communication between WarehousePG Database segment instance worker processes. The information can be helpful when debugging network issues between segment instances.
...
Specifies the flow control method used for the default WarehousePG Database UDPIFC interconnect.
...
Sets the proxy ports that WarehousePG Database uses when the server configuration parameter [gp\_interconnect\_type](#gp_interconnect_type) is set to `proxy`. Otherwise, this parameter is ignored. The default value is an empty string \(""\).
...
You must specify the value as a single-quoted string. This `gpconfig` command sets the value for `gp_interconnect_proxy_addresses` as a single-quoted string. The WarehousePG system consists of a coordinator and a single segment instance.
...
Specifies the amount of time, in seconds, that WarehousePG Database waits for the interconnect to complete setup before it times out.
...
Specifies the amount of time, in seconds, that WarehousePG Database waits for network transmission of interconnect traffic to complete before it times out.
...
Sets the networking protocol used for WarehousePG Database interconnect traffic. UDPIFC specifies using UDP with flow control for interconnect traffic, and is the only value supported.
...
With TCP as the interconnect protocol, WarehousePG Database has an upper limit of 1000 segment instances - less than that if the query workload involves complex, multi-slice queries.
...
Sets the tuple-serialization chunk size for the WarehousePG Database interconnect.
...
Specifies the maximum number of active parallel retrieve cursors allowed on a WarehousePG Database cluster. A parallel retrieve cursor is considered active after it has been `DECLARE`d, but before it is `CLOSE`d or returns an error.
...
Specifies the maximum number of slices \(portions of a query plan that are run on segment instances\) that can be generated by a query. If the query generates more than the specified number of slices, WarehousePG Database returns an error and does not run the query. The default value is `0`, no maximum value.

Running a query that generates a large number of slices might affect WarehousePG Database performance. For example, a query that contains `UNION` or `UNION ALL` operators over several complex views can generate a large number of slices. You can run `EXPLAIN ANALYZE` on the query to view slice statistics for the query.
...
When starting a cluster, this server configuration parameter tells a node which type of IP address to use. When set to `auto`, VMware WarehousePG loops through all the ipv4 and ipv6 addresses specified in `listen_addresses`, trying to open a “listening” port on each one.
...
When a user starts a session with WarehousePG Database and issues a query, the system creates groups or 'gangs' of worker processes on each segment to do the work. `gp_print_create_gang_time` controls the display of additional information about gang creation, including gang reuse status and the shortest and longest connection establishment time to the segment.

The default value is `false`, WarehousePG Database does not display the additional gang creation information.
...
Determines how WarehousePG Database handles legacy SQL that specifies a compression type of QuickLZ, which is not supported in WarehousePG Database 7 and later. When set to `true`, if legacy SQL specifies a compression type of QuickLZ, WarehousePG Database will apply a compression type of zstd. When set to `false` -- the default -- specifying a compression type of QuickLZ will result in an error.
...
When set to `auto`, WarehousePG Database uses resource group memory limits to distribute memory across query operators, allocating a fixed size of memory to non-memory-intensive operators and the rest to memory-intensive operators.

When you specify `eager_free`, WarehousePG Database distributes memory among operators more optimally by re-allocating memory released by operators that have completed their processing to operators in a later query stage.
...
> **Note** The `gp_resource_group_bypass` server configuration parameter is enforced only when resource group-based resource management is active.
 ...
 Activates or deactivates  the enforcement of resource group concurrent transaction limits on WarehousePG Database resources. The default value is `false`, which enforces resource group transaction limits. Resource groups manage resources such as CPU, memory, and the number of concurrent transactions that are used by queries.
...
When set to `true` -- the default -- WarehousePG Database's resource group scheduler bypasses all queries that fulfill both of the following criteria:
...
Identifies the maximum percentage of system CPU resources to allocate to resource groups on each WarehousePG Database segment node.
...
Identifies the resource management scheme currently enabled in the WarehousePG Database cluster. For information about WarehousePG Database resource management, see [Managing Resources](../../admin_guide/wlmgmt.html).

- `none` - configures WarehousePG Database to not use any resource manager. This is the default.

- `group` - configures WarehousePG Database to use resource groups and base resource group behavior on the cgroup v1 version of Linux cgroup functionality. 

- `group-v2` - configures WarehousePG Database to use resource groups and base resource group behavior on the cgroup v2 version of Linux cgroup functionality. 

- `queue` - configures WarehousePG Database to use resource queues. 
...
Enables WarehousePG memory management features. The distribution algorithm `eager_free` takes advantage of the fact that not all operators run at the same time\(in WarehousePG Database 4.2 and later\). The query plan is divided into stages and WarehousePG Database eagerly frees memory allocated to a previous stage at the end of that stage's execution, then allocates the eagerly freed memory to the new stage.

When set to `none`, memory management is the same as in WarehousePG Database releases prior to 4.1.
...
> **Note** This parameter is deprecated and will be removed in a future WarehousePG Database release.
...
Setting the value to `true` uses the modulo hash algorithm that is compatible with WarehousePG Database 5.x and earlier releases.
...
First calculate the value of `gp_vmem`. This is the WarehousePG Database memory available on a host.
...
Based on the `gp_vmem` value you can calculate the value for the `vm.overcommit_ratio` operating system kernel parameter. This parameter is set when you configure each WarehousePG Database host.
...
For information about the kernel parameter, see the *WarehousePG Database Installation Guide*.
...
If your WarehousePG Database installation uses serial ATA \(SATA\) disk drives, enabling compression might help to avoid overloading the disk subsystem with IO operations.
...
Controls the time \(in seconds\) that WarehousePG Database waits before returning an error when WarehousePG Database is attempting to connect or write to a [gpfdist](../../utility_guide/ref/gpfdist.html) server and `gpfdist` does not respond. The default value is 300 \(5 minutes\). A value of 0 deactivates the timeout.
...
WarehousePG Database uses checksums to prevent loading data that has been corrupted in the file system into memory managed by database processes.

By default, when a checksum verify error occurs when reading a heap data page, WarehousePG Database generates an error and prevents the page from being loaded into managed memory. When `ignore_checksum_failure` is set to on and a checksum verify failure occurs, WarehousePG Database generates a warning, and allows the page to be read into managed memory. If the page is then updated it is saved to disk and replicated to the mirror. If the page header is corrupt an error is reported even if this option is enabled.
...
Determines whether JIT compilation may be used by WarehousePG Database. It enables or disables JIT for both GPORCA and Postgres-based planner.
...
Reports the locale in which sorting of textual data is done. The value is determined when the WarehousePG Database array is initialized.
...
Reports the locale that determines character classifications. The value is determined when the WarehousePG Database array is initialized.
...
WarehousePG Database uses the [deadlock\_timeout](#deadlock_timeout) and [gp\_global\_deadlock\_detector\_period](#gp_global_deadlock_detector_period) to trigger local and global deadlock detection. Note that if `lock_timeout` is turned on and set to a value smaller than these deadlock detection timeouts, WarehousePG Database will abort a statement before it would ever trigger a deadlock check in that session.
...
By default, connection log messages only show the IP address of the connecting host. Turning on this option causes logging of the IP address and host name of the WarehousePG Database coordinator. Note that depending on your host name resolution setup this might impose a non-negligible performance penalty.
...
If the WarehousePG Database PL/Container extension is installed. This parameter also controls the PL/Container log level. For information about the extension, see [PL/pgSQL Language](../../analytics/pl_container.html).
...
Determines the amount of time WarehousePG Database writes messages to the active log file. When this amount of time has elapsed, the file is closed and a new log file is created. Set to zero to deactivate time-based creation of new log files.
...
The maximum number of concurrent connections to the database server. In a WarehousePG Database system, user client connections go through the WarehousePG coordinator instance only. Segment instances should allow 3-10 times the amount as the coordinator. When you increase this parameter, [max\_prepared\_transactions](#max_prepared_transactions) must be increased as well. For more information about limiting concurrent connections, see "Configuring Client Authentication" in the *WarehousePG Database Administrator Guide*.

Increasing this parameter may cause WarehousePG Database to request more shared memory. See [shared\_buffers](#shared_buffers) for information about WarehousePG server instance shared memory buffers.
...
Sets the maximum number of resource queues that can be created in a WarehousePG Database system. Note that resource queues are system-wide \(as are roles\) so they apply to all databases in the system.
...
In WarehousePG Database, replication slots exist internally by default for primary/mirror pairs. Consequently, even if you do not configure this parameter, that functionality is available by default. 
...
Sets the WAL size at which to trigger a checkpoint. This is the maximum size WarehousePG Database allows the WAL to grow during automatic checkpoints. This is a soft limit; WAL size can exceed `max_wal_size` under special circumstances, such as heavy load, a failing archive_command, or a high `wal_keep_segments` setting.
...
Sets the minimum size to which to shrink the WAL. As long as WAL disk usage stays below this setting, WarehousePG Database always recycles old WAL files for future use at a checkpoint, rather than remove them. You can use this parameter to ensure that enough WAL space is reserved to handle spikes in WAL usage, for example when running large batch jobs.
...
## <a id="optimizer"></a>optimizer 
 ...
 Activates or deactivates  GPORCA when running SQL queries. The default is `on`. If you deactivate GPORCA, WarehousePG Database uses only the Postgres-based planner.

GPORCA co-exists with the Postgres-based planner. With GPORCA enabled, WarehousePG Database uses GPORCA to generate an execution plan for a query when possible. If GPORCA cannot be used, then the Postgres-based planner is used.

The optimizer parameter can be set for a database system, an individual database, or a session or query.

For information about the Postgres-based planner and GPORCA, see [SQL: Querying Data](../../admin_guide/query/topics/query.html) in the *WarehousePG Database Administrator Guide*.
...
-   `legacy` - preserves the calibrated cost model used by GPORCA in WarehousePG Database releases 6.13 and earlier
...
For information about GPORCA, see [About GPORCA](../../admin_guide/query/topics/query-piv-optimizer.html) in the *WarehousePG Database Administrator Guide*.
...
When GPORCA is enabled \(the default\) and this parameter is `true` \(the default\), GPORCA attempts to run DML commands such as `INSERT`, `UPDATE`, and `DELETE`. If GPORCA cannot run the command, WarehousePG Database falls back to the Postgres-based planner.

When set to `false`, WarehousePG Database always falls back to the Postgres-based planner when performing DML commands.
...
When GPORCA is enabled \(the default\), this parameter allows GPORCA to run catalog queries that run only on the WarehousePG Database coordinator. For the default value `off`, only the Postgres-based planner can run catalog queries that run only on the WarehousePG Database coordinator.
...
Sets the maximum amount of memory on the WarehousePG Database coordinator that GPORCA uses to cache query metadata \(optimization data\) during query optimization. The memory limit session based. GPORCA caches query metadata during query optimization with the default settings: GPORCA is enabled and [optimizer\_metadata\_caching](#optimizer_metadata_caching) is `on`.
...
When GPORCA is enabled \(the default\), this parameter specifies whether GPORCA caches query metadata \(optimization data\) in memory on the WarehousePG Database coordinator during query optimization. The default for this parameter is `on`, enable caching. The cache is session based. When a session ends, the cache is released. If the amount of query metadata exceeds the cache size, then old, unused metadata is evicted from the cache.
...
When GPORCA is enabled \(the default\) and this parameter is `true` \(the default\), GPORCA uses WarehousePG Database memory management when running queries. When set to `false`, GPORCA uses GPORCA-specific memory management. WarehousePG Database memory management allows for faster optimization, reduced memory usage during optimization, and improves GPORCA support of vmem limits when compared to GPORCA-specific memory management.
...
The database listener port for a WarehousePG instance. The coordinator and each segment has its own port. Port numbers for the WarehousePG system must also be changed in the `gp_segment_configuration` catalog. You must shut down your WarehousePG Database system before changing port numbers.
...
When an SQL query reads from an external table, the parameter value specifies the amount of time in seconds that WarehousePG Database waits before cancelling the query when data stops being returned from the external table.

The default value of 0, specifies no time out. WarehousePG Database does not cancel the query.

If queries that use gpfdist run a long time and then return the error "intermittent network connectivity issues", you can specify a value for `readable_external_table_timeout`. If no data is returned by gpfdist for the specified length of time, WarehousePG Database cancels the query.
...
For WarehousePG Database coordinator mirroring, controls updates to the active coordinator. If the number of WAL segment files that have not been processed by the `walsender` exceeds this value, WarehousePG Database updates the active coordinator.

If the number of segment files does not exceed the value, WarehousePG Database blocks updates to the to allow the `walsender` process the files. If all WAL segments have been processed, the active coordinator is updated.
...
For WarehousePG Database coordinator mirroring, sets the maximum time in milliseconds that the `walsender` process on the active coordinator waits for a status message from the `walreceiver` process on the standby coordinator. If a message is not received, the `walsender` logs an error message.
...
For queries that are managed by resource queues or resource groups, this parameter determines when WarehousePG Database terminates running queries based on the amount of memory the queries are using. A value of 100 deactivates the automatic termination of queries based on the percentage of memory that is utilized.

Either the resource queue or the resource group management scheme can be active in WarehousePG Database; both schemes cannot be active at the same time. The server configuration parameter [gp\_resource\_manager](#gp_resource_manager) controls which scheme is active.

This parameter sets the percent of utilized WarehousePG Database virtual memory that triggers the termination of queries. If the percentage of virtual memory that is utilized for a WarehousePG Database segment exceeds the specified value, WarehousePG Database terminates queries managed by resource queues based on memory usage, starting with the query consuming the largest amount of memory. Queries are terminated until the percentage of utilized virtual memory is below the specified percentage.

Specify the maximum virtual memory value for active WarehousePG Database segment instances with the server configuration parameter [gp\_vmem\_protect\_limit](#gp_vmem_protect_limit).

For example, if `gp_vmem_protect_limit` is set to 10GB, and `runaway_detector_activation_percent` is 90 \(90%\), WarehousePG Database starts terminating queries when the utilized virtual memory exceeds 9 GB.
...
Reports the database encoding \(character set\). It is determined when the WarehousePG Database array is initialized. Ordinarily, clients need only be concerned with the value of *client\_encoding*.
...
Reports the version of PostgreSQL that this release of WarehousePG Database is based on.
...
Reports the version of PostgreSQL that this release of WarehousePG Database is based on as an integer.
...
Sets the amount of memory a WarehousePG Database segment instance uses for shared memory buffers. This setting must be at least 128KB and at least 16KB times [max\_connections](#max_connections).

Each WarehousePG Database segment instance calculates and attempts to allocate certain amount of shared memory based on the segment configuration. The value of `shared_buffers` is significant portion of this shared memory calculation, but is not all it. When setting `shared_buffers`, the values for the operating system parameters `SHMMAX` or `SHMALL` might also need to be adjusted.
...
The value of `other_seg_shmem` is the portion the WarehousePG Database shared memory calculation that is not accounted for by the `shared_buffers` value. The `other_seg_shmem` value will vary based on the segment configuration.

With the default WarehousePG Database parameter values, the value for `other_seg_shmem` is approximately 111MB for WarehousePG Database segments and approximately 79MB for the WarehousePG Database coordinator.
...
See the *WarehousePG Database Installation Guide* for information about the WarehousePG Database values for the parameters `SHMMAX` and `SHMALL`.
...
*If you are using resource groups to control resource allocation in your WarehousePG Database cluster*:
...
*If you are using resource queues to control resource allocation in your WarehousePG Database cluster*:
...
Determines the number of connection slots that are reserved for WarehousePG Database superusers.
...
To configure WarehousePG Database to use a custom collection of timezones, copy the file that contains the timezone definitions to the directory `$GPHOME/share/postgresql/timezonesets/` on the WarehousePG Database coordinator and segment hosts. Then set value of the server configuration parameter `timezone_abbreviations` to the file. For example, to use a file `custom` that contains the default timezones and the WIB \(Waktu Indonesia Barat\) timezone.
...
2.  Copy the file `custom` to the directory `$GPHOME/share/postgresql/timezonesets/` on the WarehousePG Database coordinator and segment hosts.
...
> **Important** Do not change the value of this parameter. The default location is required for WarehousePG Database utilities.
...
For information about `VACUUM` and transaction ID management, see "Managing Data" in the *WarehousePG Database Administrator Guide* and the [PostgreSQL documentation](https://www.postgresql.org/docs/12/routine-vacuuming.html#VACUUM-FOR-WRAPAROUND).
...
Enables a test that validates the free tuple ID \(TID\) list. The list is maintained and used by WarehousePG Database. WarehousePG Database determines the validity of the free TID list by ensuring the previous free TID of the current free tuple is a valid free tuple. The default value is `true`, enable the test.

If WarehousePG Database detects a corruption in the free TID list, the free TID list is rebuilt, a warning is logged, and a warning is returned by queries for which the check failed. WarehousePG Database attempts to run the queries.
...
When a WarehousePG Database external table is defined with the `gpfdists` protocol to use SSL security, this parameter controls whether SSL certificate authentication is enabled.

Regardless of the setting of this server configuration parameter, WarehousePG Database always encrypts data that you read from or write to an external table that specifies the `gpfdists` protocol.

The default is `true`, SSL certificate authentication is enabled when WarehousePG Database communicates with the `gpfdist` utility to either read data from or write data to an external data source.

The value `false` deactivates SSL certificate authentication. These SSL exceptions are ignored:

-   The self-signed SSL certificate that is used by `gpfdist` is not trusted by WarehousePG Database.
-   The host name contained in the SSL certificate does not match the host name that is running `gpfdist`.

When you set `verify_gpfdists_cert` to `false`, the CA certification file is not required for WarehousePG Database segments.

You can set the value to `false` to deactivate authentication when testing the communication between the WarehousePG Database external table and the `gpfdist` utility that is serving the external data.
...
For information about the `gpfdists` protocol and how this setting affects the certificate files required on WarehousePG Database segments, see [gpfdists:// Protocol](../../admin_guide/external/gpfdists-protocol.html). For information about running the `gpfdist` utility, see [gpfdist](../../utility_guide/ref/gpfdist.html).
...
When WarehousePG Database segment mirroring is enabled, specifies the maximum amount of Write-Ahead Logging \(WAL\)-based records \(in KB\) written by a transaction on the primary segment instance before the records are written to the mirror segment instance for replication. As the default, WarehousePG Database writes the records to the mirror segment instance when a checkpoint occurs or the `wait_for_replication_threshold` value is reached.
...
For WarehousePG Database coordinator mirroring, sets the interval in seconds between `walreceiver` process status messages that are sent to the active coordinator. Under heavy loads, the time might be longer.
...
Size of the buffer that WarehousePG Database uses for network communication, such as the `gpfdist` utility and external web tables \(that use http\). Valid units are `KB` \(as in `128KB`\), `MB`, `GB`, and `TB`. WarehousePG Database stores data in the buffer before writing the data out. For information about `gpfdist`, see the *WarehousePG Database Utility Guide*.
...
The number of transaction IDs prior to the ID where transaction ID wraparound occurs. When this limit is reached, WarehousePG Database stops creating new transactions to avoid data loss due to transaction ID wraparound.
...
The number of transaction IDs prior to the limit specified by [xid\_stop\_limit](#xid_stop_limit). When WarehousePG Database reaches this limit, it issues a warning to perform a VACUUM operation to avoid data loss due to transaction ID wraparound.
...
The parameter can be set for a database system, an individual database, or a session. The SQL command to set this option for a session is also available in WarehousePG Database.
