...
> **Note** WarehousePG Database does not enforce referential integrity syntax \(foreign key constraints\). `TRUNCATE` truncates a table that is referenced in a foreign key constraint even if the `CASCADE` option is omitted.
...
CASCADE
:   Because this key word applies to foreign key references \(which are not supported in WarehousePG Database\) it has no effect.
