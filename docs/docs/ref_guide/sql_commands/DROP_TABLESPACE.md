...
IF EXISTS
:   Do not throw an error if the tablespace does not exist. WarehousePG Database issues a notice in this case.
...
`DROP TABLESPACE` is a WarehousePG Database extension.
