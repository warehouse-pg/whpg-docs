...
> **Note**
> The default sequence cache size in WarehousePG Database is `20`.
...
IF NOT EXISTS
:   Do not throw an error if a relation with the same name already exists. WarehousePG Database issues a notice in this case. Note that there is no guarantee that the existing relation is anything like the sequence that would have been created - it might not even be a sequence.
...
:   > **Note** When operating with a cache of sequence numbers (`cache > 1`), WarehousePG Database may discard some cached sequence values. If you require consecutive values, you must explicitly set `CACHE 1` when you create or alter the sequence.
...
Reset the sequence counter value on the WarehousePG Database coordinator:

```
SELECT setval('myseq', 201);
```

Illegal use of `setval()` in WarehousePG Database \(setting sequence values on distributed data\):
...
-   The `OWNED BY` clause is a WarehousePG Database extension.
