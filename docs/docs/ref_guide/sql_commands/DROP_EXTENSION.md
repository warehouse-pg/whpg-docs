...
Removes an extension from a WarehousePG database.
...
> **Note** The supporting extension files that were installed to create the extension \(for example, the library and `.control` files\) are not deleted. The files must be manually removed from the WarehousePG Database hosts.
...
IF EXISTS
:   Do not throw an error if the extension does not exist. WarehousePG Database issues a notice in this case.
...
`DROP EXTENSION` is a WarehousePG Database extension.
