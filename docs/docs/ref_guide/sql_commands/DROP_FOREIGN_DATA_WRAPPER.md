...
IF EXISTS
:   Do not throw an error if the foreign-data wrapper does not exist. WarehousePG Database issues a notice in this case.
...
`DROP FOREIGN DATA WRAPPER` conforms to ISO/IEC 9075-9 \(SQL/MED\). The `IF EXISTS` clause is a WarehousePG Database extension.
