...
It is very important to take care to avoid circular rules. For example, though each of the following two rule definitions are accepted by WarehousePG Database, the `SELECT` command would cause WarehousePG to report an error because of recursive expansion of a rule:
...
`CREATE RULE` is a WarehousePG Database extension, as is the entire query rewrite system.
