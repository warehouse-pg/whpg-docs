...
> **Note**
> Because WarehousePG Database does not support scrollable cursors, the `RETRIEVE` command moves a parallel retrieve cursor only forward in position.
...
`RETRIEVE` is a WarehousePG Database extension. The SQL standard makes no provisions for parallel retrieve cursors.
