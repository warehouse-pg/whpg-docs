...
Note that in WarehousePG Database the shared library file \(`.so` file\) must reside in the same path location on every host in the WarehousePG Database array \(coordinators, segments, and mirrors\).
...
filename
:   The path and file name of a shared library file. This file must exist in the same location on all hosts in your WarehousePG Database array.
...
`LOAD` is a WarehousePG Database extension.
