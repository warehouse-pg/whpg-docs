...
`CREATE TABLESPACE` registers and configures a new tablespace for your WarehousePG Database system. The tablespace name must be distinct from the name of any existing tablespace in the system. A tablespace is a WarehousePG Database system object \(a global object\), you can use a tablespace from any database if you have appropriate privileges.
...
A user with appropriate privileges can pass tablespace\_name to [CREATE DATABASE](CREATE_DATABASE.html), [CREATE TABLE](CREATE_TABLE.html), or [CREATE INDEX](CREATE_INDEX.html) to direct WarehousePG Database to store the data files for these objects within the specified tablespace.

In WarehousePG Database, the file system location must exist on all hosts including the hosts running the coordinator, standby mirror, each primary segment, and each mirror segment.
...
LOCATION 'directory'
:   The directory that will be used for the tablespace. The directory should be empty and must be owned by the WarehousePG Database system user. You must specify the absolute path of the directory, and the path name must not be greater than 100 characters in length. \(The location is used to create a symlink target in the pg\_tblspc directory, and symlink targets are truncated to 100 characters when sending to `tar` from utilities such as `pg_basebackup`.\)

:   You can specify a different tablespace directory for any WarehousePG Database segment instance in the `WITH` clause.
...
:   If a segment instance is not listed in the `WITH` clause, WarehousePG Database uses the tablespace directory specified in the `LOCATION` clause.
...
Because `CREATE TABLESPACE` creates symbolic links from the `pg_tblspc` directory in the coordinator and segment instance data directory to the directories specified in the command, WarehousePG Database supports tablespaces only on systems that support symbolic links.
...
`CREATE TABLESPACE` is a WarehousePG Database extension.
