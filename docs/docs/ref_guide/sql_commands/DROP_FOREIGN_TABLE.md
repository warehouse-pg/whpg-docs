...
IF EXISTS
:   Do not throw an error if the foreign table does not exist. WarehousePG Database issues a notice in this case.
...
`DROP FOREIGN TABLE` conforms to ISO/IEC 9075-9 \(SQL/MED\), except that the standard only allows one foreign table to be dropped per command. The `IF EXISTS` clause is a WarehousePG Database extension.
