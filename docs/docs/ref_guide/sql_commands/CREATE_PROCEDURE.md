...
A `CREATE PROCEDURE` command is defined in the SQL standard. The WarehousePG Database version is similar but not fully compatible. For details see also [CREATE FUNCTION](CREATE_FUNCTION.html).
