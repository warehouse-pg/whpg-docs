...
The `CREATE POLICY` command defines a new row-level security policy for a table. Note that row-level security must be enabled on the table \(using `ALTER TABLE ... ENABLE ROW LEVEL SECURITY`\) in order for WarehousePG Database to apply created policies.
...
`CREATE POLICY` is a WarehousePG Database extension to the SQL standard.
