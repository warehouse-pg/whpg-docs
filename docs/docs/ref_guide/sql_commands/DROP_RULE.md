...
IF EXISTS
:   Do not throw an error if the rule does not exist. WarehousePG Database issues a notice in this case.
...
`DROP RULE` is a WarehousePG Database extension, as is the entire query rewrite system.
