...
Plans should be read from the bottom up as each node feeds rows into the node directly above it. The bottom nodes of a plan are usually table scan operations \(sequential, index or bitmap index scans\). If the query requires joins, aggregations, or sorts \(or other operations on the raw rows\) then there will be additional nodes above the scan nodes to perform these operations. The topmost plan nodes are usually the WarehousePG Database motion nodes \(redistribute, explicit redistribute, broadcast, or gather motions\). These are the operations responsible for moving rows between the segment instances during query processing.
...
> **Note**
> WarehousePG Database does not support specifying `BUFFERS [true]` for distributed queries; ignore any displayed buffer usage information.
...
An SQL statement that is run during the execution of an `EXPLAIN ANALYZE` command is excluded from WarehousePG Database resource queues.

For more information about query profiling, see "Query Profiling" in the *WarehousePG Database Administrator Guide*. For more information about resource queues, see "Resource Management with Resource Queues" in the *WarehousePG Database Administrator Guide*.
...
The results of the scan operation are passed up to a *gather motion* operation. In WarehousePG Database, a gather motion is when segments send rows up to the coordinator. In this case we have 3 segment instances sending to 1 coordinator instance \(3:1\). This operation is working on `slice1` of the parallel query execution plan. In WarehousePG Database a query plan is divided into *slices* so that portions of the query plan can be worked on in parallel by the segments.
