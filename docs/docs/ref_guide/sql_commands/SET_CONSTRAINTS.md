...
This command complies with the behavior defined in the SQL standard, except for the limitation that, in WarehousePG Database, it does not apply to `NOT NULL` and `CHECK` constraints. Also, WarehousePG Database checks non-deferrable uniqueness constraints immediately, not at end of statement as the standard would suggest.
