...
:   PRESERVE ROWS — WarehousePG Database takes no special action at the ends of transactions for temporary tables. This is the default behavior.

:   DELETE ROWS — WarehousePG Database deletes all rows in the temporary table at the end of each transaction block. Essentially, an automatic [TRUNCATE](TRUNCATE.html) is done at each commit.

:   DROP — WarehousePG Database drops the temporary table at the end of the current transaction block.
...
DISTRIBUTED RANDOMLY
DISTRIBUTED REPLICATED
:   Used to declare the WarehousePG Database distribution policy for the table. Refer to [CREATE TABLE](CREATE_TABLE.html) for details.
...
-   The standard requires parentheses around the subquery clause; in WarehousePG Database, these parentheses are optional.
-   In the standard, the `WITH [NO] DATA` clause is required, in WarehousePG Database it is optional.
-   WarehousePG Database handles temporary tables differently from the standard; see [CREATE TABLE](CREATE_TABLE.html) for details.
-   The `WITH` clause is a WarehousePG Database extension; storage parameters are not part of the standard.
-   The WarehousePG Database concept of tablespaces is not part of the standard. The `TABLESPACE` clause is an extension.
