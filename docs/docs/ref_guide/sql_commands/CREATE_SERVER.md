...
:   Do not throw an error if a server with the same name already exists. WarehousePG Database issues a notice in this case. Note that there is no guarantee that the existing server is anything like the one that would have been created.
...
num\_segments 'num'
:   When `mpp_execute` is set to `'all segments'`, the WarehousePG Database-specific `num_segments` option identifies the number of query executors that WarehousePG Database spawns on the source WarehousePG Database cluster. If you do not provide a value, num defaults to the number of segments in the source cluster.
