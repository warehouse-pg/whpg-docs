...
When you alter a memory quota of a resource group that you create for roles, the new resource limit is immediately applied if current resource usage is less than or equal to the new value and there are no running transactions in the resource group. If the current resource usage exceeds the new memory quota value, or if there are running transactions in other resource groups that hold some of the resource, then WarehousePG Database defers assigning the new limit until resource usage falls within the range of the new value.
...
The `ALTER RESOURCE GROUP` statement is a WarehousePG Database extension. This command does not exist in standard PostgreSQL.
