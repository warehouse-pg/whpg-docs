...
If multiple savepoints have the same name, WarehousePG Database releases only the most recently defined unreleased savepoint. Repeated commands release progressively older savepoints.
...
This command conforms to the SQL standard. The standard specifies that the key word `SAVEPOINT` is mandatory, but WarehousePG Database allows it to be omitted.
