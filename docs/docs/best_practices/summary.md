...
WarehousePG Database is an analytical MPP shared-nothing database. This model is significantly different from a highly normalized/transactional SMP database. Because of this, the following best practices are recommended.

-   WarehousePG Database performs best with a denormalized schema design suited for MPP analytical processing for example, Star or Snowflake schema, with large fact tables and smaller dimension tables.
...
-   You can use `gp_vmem_protect_limit` by calculating:
    -   `gp_vmem` – the total memory available to WarehousePG Database
...
-   In general indexes are not needed in WarehousePG Database.
...
See [System Monitoring and Maintenance](maintenance.html), [Query Profiling](../admin_guide/query/topics/query-profiling.html#in198649) and [Monitoring WarehousePG Database Log Files](logfiles.html).
...
-   Limit users who have the `SUPERUSER` role attribute. Roles that are superusers bypass all access privilege checks in WarehousePG Database, as well as resource queuing. Only system administrators should be given superuser rights. See "Altering Role Attributes" in the *WarehousePG Database Administrator Guide*.
-   Database users should never log on as `gpadmin`, and ETL or production workloads should never run as `gpadmin`.
-   Assign a distinct WarehousePG Database role to each user, application, or service that logs in.
...
-   Backup WarehousePG databases regularly unless the data is easily restored from sources.
