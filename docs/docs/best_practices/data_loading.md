...
External tables provide access to data in sources outside of WarehousePG Database. They can be accessed with `SELECT` statements and are commonly used with the Extract, Load, Transform \(ELT\) pattern, a variant of the Extract, Transform, Load \(ETL\) pattern that takes advantage of WarehousePG Database's fast parallel data loading capability.
...
See the *WarehousePG Database Reference Guide* for detailed instructions for loading data using `gpfdist` and `gpload`.
