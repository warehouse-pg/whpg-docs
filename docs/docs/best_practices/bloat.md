...
If you are performing system catalog maintenance during a maintenance period and you need to stop a process due to time constraints, run the WarehousePG Database function `pg_cancel_backend(<PID>)` to safely stop a WarehousePG Database process.
