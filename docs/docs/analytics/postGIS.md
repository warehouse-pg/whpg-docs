...
For information about WarehousePG Database PostGIS extension support, see [PostGIS Extension Support and Limitations](#postgis_support).
...
The WarehousePG PostGIS extension package is available from [VMware Tanzu Network](https://network.tanzu.vmware.com/products/vmware-greenplum). After you download the package, you can follow the instructions in [Verifying the WarehousePG Database Software Download](../install_guide/verify_sw.html) to verify the integrity of the download. You can install the package using the WarehousePG Package Manager (`gppkg`). For details, see [`gppkg`](../utility_guide/ref/gppkg.html) in the _WarehousePG Database Utility Guide_.

WarehousePG Database supports the PostGIS extension with these component versions:
...
To enable PostGIS support, install the WarehousePG PostGIS extension package into the WarehousePG Database system, and then use the `CREATE EXTENSION` command to enable PostGIS support for an individual database.
...
After installing the package, source the `greenplum_path.sh` file and restart WarehousePG Database. This command restarts WarehousePG Database.

```
gpstop -ra
```

Installing the WarehousePG PostGIS extension package updates the WarehousePG Database system, including installing the supported PostGIS extensions to the system and updating `greenplum_path.sh` file with these lines for PostGIS Raster support.
...
PostGIS uses GDAL raster drivers when processing raster data with commands such as `ST_AsJPEG()`. As the default, PostGIS deactivates all raster drivers. You enable raster drivers by setting the value of the `POSTGIS_GDAL_ENABLED_DRIVERS` environment variable in the `greenplum_path.sh` file on all WarehousePG Database hosts.

Alternatively, you can do it at the session level by setting `postgis.gdal_enabled_drivers`. For a WarehousePG Database session, this example `SET` command enables three GDAL raster drivers.
...
To see the list of supported GDAL raster drivers for a WarehousePG Database system, run the `raster2pgsql` utility with the `-G` option on the WarehousePG Database coordinator.
...
The `gpstop -r` command restarts the WarehousePG Database system to use the updated settings in the `greenplum_path.sh` file.

After you have updated the `greenplum_path.sh` file on all hosts, and have restarted the WarehousePG Database system, you can display the enabled raster drivers with the `ST_GDALDrivers()` function. This `SELECT` command lists the enabled raster drivers.
...
After installing PostGIS, the default setting `POSTGIS_ENABLE_OUTDB_RASTERS=0` in the `greenplum_path.sh` file deactivates support for out-of-database rasters. To enable this feature, you can set the value to true \(a non-zero value\) on all hosts and restart the WarehousePG Database system.

You can also activate or deactivate this feature for a WarehousePG Database session. For example, this `SET` command enables the feature for the current session.
...
After PostGIS support has been removed from all databases in the WarehousePG Database system, you can remove the PostGIS extension package. For example, this `gppkg` command removes the PostGIS extension package.
...
Source the `greenplum_path.sh` file and restart WarehousePG Database. This command restarts WarehousePG Database.
...
-   Data types and functions related to PostGIS topology functionality, such as TopoGeometry, are not supported by WarehousePG Database.
-   These PostGIS aggregates are not supported by WarehousePG Database:

    -   ST\_Collect
    -   ST\_MakeLine
    
    On a WarehousePG Database with multiple segments, the aggregate might return different answers if it is called several times repeatedly.

-   WarehousePG Database does not support PostGIS long transactions.

    PostGIS relies on triggers and the PostGIS table `public.authorization_table` for long transaction support. When PostGIS attempts to acquire locks for long transactions, WarehousePG Database reports errors citing that the function cannot access the relation, `authorization_table`.

-   The `_postgis_index_extent` function is not supported on WarehousePG Database 7 due to its dependence on spatial index operations.
-   The `<->` operator \(`geometry <-> geometry`\) returns the centroid/centroid distance for WarehousePG Database 7.
