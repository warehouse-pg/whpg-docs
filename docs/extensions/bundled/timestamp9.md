---
title: timestamp9
description: Provides an efficient, nanosecond-precision timestamp data type and related functions.
redirects:
  - /docs/6x/ref_guide/modules/bundled/timestamp9
---
...
If no time zone is specified in the input string, then it is presumed to be in the time zone indicated by the system's [`TIMEZONE` server configuration parameter](/docs/6x/ref_guide/config_params/guc-list.md#timezone) and is converted to UTC using the offset for the time zone.
...
You can set the [TimeZone](/docs/6x/ref_guide/config_params/guc-list.md#timezone) server configuration parameter to specify the time zone that WarehousePG uses when it prints a `timestamp9` timestamp. When you set this parameter, WarehousePG displays the timestamp value in that time zone. For example:
