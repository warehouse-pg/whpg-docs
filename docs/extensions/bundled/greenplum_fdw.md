...
title: greenplum_fdw
redirects:
  - /docs/6x/ref_guide/modules/bundled/greenplum_fdw
...
Refer to [Installing Extensions](/docs/6x/install_guide/additional_modules/index.md) for more information about installing and registering modules in WarehousePG.
...
`greenplum_fdw` depends on the [gp_parallel_retrieve_cursor](gp_parallel_retrieve_cursor.md) module.
...
To access a remote WarehousePG cluster, you must first create a foreign server object which specifies the host, port, and database connection details. You provide these connection parameters in the `OPTIONS` clause of the [CREATE SERVER](/docs/6x/ref_guide/sql_commands/CREATE_SERVER.md) command.
...
After you identify which users you will allow to access the remote WarehousePG cluster, you must create one or more mappings between a local WarehousePG user and a user on the remote WarehousePG cluster. You create these mappings with the [CREATE USER MAPPING](/docs/6x/ref_guide/sql_commands/CREATE_USER_MAPPING.md) command.
