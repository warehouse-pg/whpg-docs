---
title: postgres_fdw
description: Provides a foreign-data wrapper for accessing data stored in a remote PostgreSQL or WarehousePG database.
redirects:
  - /docs/6x/ref_guide/modules/bundled/postgres_fdw
  - /docs/7x/ref_guide/modules/bundled/postgres_fdw
  - /docs/7x/ref_guide/modules/postgres_fdw
---
...
The `postgres_fdw` module is installed when you install WarehousePG. Before you can use the foreign-data wrapper, you must register the `postgres_fdw` extension in each database in which you want to use the foreign-data wrapper. Refer to [Installing Extensions](/docs/7x/install_guide/additional_modules/index.md) for more information.
...
Before you [create a foreign table](/docs/7x/ref_guide/sql_commands/CREATE_FOREIGN_TABLE.md) using the `postgres_fdw` foreign-data wrapper, you must configure a server with the [CREATE SERVER](/docs/7x/ref_guide/sql_commands/CREATE_SERVER.md) command. You can use a `postgres_fdw` foreign table to access data that is distributed across multiple remote PostgreSQL servers when you set certain WarehousePG and `postgres_fdw`-specific options on the `CREATE SERVER` command:
...
For more information about using foreign tables in WarehousePG, see [Accessing External Data with Foreign Tables](/docs/7x/admin_guide/external/foreign/index.md).
